...
BYTE code tVScalingCoeff[ 6 ][ V_COEF_NUM ] =
{
	...
	//{ 0x1D, 0x16, 0x10, 0x0B, 0x08, 0x46, 0x45, 0x42, 0x3E, 0x38, 0x1D, 0x24, 0x2B, 0x32, 0x38, 0x00, 0x01, 0x03, 0x05, 0x08 }, /* Fc50 */ /* Coef */
...
void mStar_SetScalingFactor( void )
{
    ...
    //DWORD xdata Factor_height;
    WORD xdata Expansionwidth;
    WORD xdata Hstar, width;
    
    DB_Mode(eDB_SUCCESSIVE_MODE);
    
    height = g_SetupPathInfo.wImgSizeInV;
    width = g_SetupPathInfo.wImgSizeInH;

    // 120531 coding reserved for correcting formula
    //Factor_height = (( DWORD )PanelHeight * 100 ) / height;
    //Expansionwidth = PANEL_WIDTH;

    if(!EXPANSION_MODE_IS_FULL() && !INPUT_IS_WILD_TIMING())//( UserprefExpansionMode == Expansion_Aspect && ExpansionFlag )
    {
        //Expansionwidth = (( DWORD )Factor_height * width ) / ( 100 );
        Expansionwidth = (DWORD)PANEL_HEIGHT*width/height;
        if(Expansionwidth > PANEL_WIDTH)
            Expansionwidth = PANEL_WIDTH;
        
        Hstar = ( PANEL_WIDTH - Expansionwidth ) / 2 + PanelHStart;
        DB_W2B( SC0_18, Hstar );
        DB_W2B( SC0_1C, Hstar + Expansionwidth - 1 );
        #if Enable_Expansion
    ...
    {
        DB_W2B( SC0_18, PanelHStart );
        DB_W2B( SC0_1C, PanelHStart + PANEL_WIDTH - 1 );
        Expansionwidth = PANEL_WIDTH;
            #if Enable_Expansion
    ...
    if( g_SetupPathInfo.wImgSizeInH != g_SetupPathInfo.wImgSizeOutH ) // set expansion factor
    {
         ...
         #else
                g_SetupPathInfo.wImgSizeOutH = Expansionwidth; // 120413 coding addition
    ...
    // Setup vertical expansion
        if( height != g_SetupPathInfo.wImgSizeOutV )
        {
                        ...
                        DB_WB(SC0_DB,factor);             
                    #else
                  //      printData("Panel Output LRInvert  = %d ",  g_SetupPathInfo.bPsvPnlOutLRInvert);
                	if(g_SetupPathInfo.b2DTo3DEnable) // VSP Non -1 mode
                	 {
                	        factor = height ;
                	        factor = (( DWORD )( factor << 20 ) ) / ( g_SetupPathInfo.wImgSizeOutV );
                	        DB_W3B( SC0_53, factor |0x800000);
        ...
        else //when 2dto3d on, the scaling function can't be disable by setting SC0_55[7]=0
            {
		if(g_SetupPathInfo.b2DTo3DEnable)
	            DB_W3B( SC0_53,0x900000);
		else
...
// 121017 coding modified
static void msSetupPathConfig()
{
    DB_Mode(eDB_NORMAL_MODE);
    //printData("aaaaaaaa g_SetupPathInfo.ucSCPathMode = %d",g_SetupPathInfo.ucSCPathMode);
        //printData("bbbbbbb g_SetupPathInfo.ucSCFmtIn= %d",g_SetupPathInfo.ucSCFmtIn);
    switch(g_SetupPathInfo.ucSCPathMode)
    {
        case SC_PATH_MODE_0:
            msWriteByteMask(SC6_02, 0x00, BIT3);
            msWriteByteMask(SC6_02, 0x00, BIT2);
            DB_WBMask(SC0_5B,IP_HSP2VSP2OP,BIT7|BIT6); // IP->VSP->OP
            DB_WBMask(REG_1E2C,BIT4|BIT0,0xFF);
            DB_WBMask(REG_1E30,0x00,0xFF);
            break;
        case SC_PATH_MODE_1:
            msWriteByteMask(SC6_02, BIT2, BIT2);
            msWriteByteMask(SC6_02, BIT3, BIT3);
...
static void msSetupPathMRWBaseWR()
{
...
#if ENABLE_MHL // 121112 coding modified
        && (V_FREQ_IN<48)
#endif        
        ) // FB mode, FRC case
    {
    ...
    {
        msWrite2Byte(SC6_D0,11);//for 3D to 2D
    }
...
static void msSetupPathMRW()
{
    ...
    msWriteBit(SC6_A3,g_SetupPathInfo.bMRW422,BIT4);// 2012/5/31 ¤U¤È 02:29:20 For RGB/YUV 444
    ...
    if(g_SetupPathInfo.bLoadOdTbl)
    {
        ...
        if(
          ...
          || (UnderScanSetting.Enable)||(g_SetupPathInfo.bFBMode)
           ...
           printData("msOverDriveOnOff=bbbbbb=FALSE==>>>>%d", 1);
           }
        else
            {
            if(g_SetupPathInfo.bFBMode)
...
void mStar_SetupPath()
{
   ...
   // msWriteByteMask(SC6_02, _BIT5, _BIT5);  //20121123 nick remove for SW PLL lock
...
#endif
   // msWriteByteMask(SC6_02, 0, _BIT5);  //20121123 nick remove for SW PLL lock
    mStar_ScalerDoubleBuffer(u8Org&BIT0);
}

void mStar_SetupPathInit()
{
    //msWriteByteMask(SC6_02,BIT4,BIT5|BIT4);  //20121123 nick Disable SW double buffer
    ...
    g_SetupPathInfo.bPsvPnlOutLRInvert =DefaultPnlOutLR;
