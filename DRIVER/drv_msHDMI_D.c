...
{
    if ( rst == 0x80 ) //Reset_HDMIStatus // 130530 JUN Modify
    {
...
{   
    ...
    if( PanelOnFlag&&(sw == HDMI_AUDIO_ON) )
    {
        hw_ClrMute();
...
void drv_EnableCTSNFileterFunction( void )
{
    ...
    msWriteByte( REG_14EC, BIT6); // clear CTS/N over range // 130530 JUN Modify
}
    ...
    if( uAudioMuteEn )
    {
        ...
        if( uAudioMuteEn & _BIT12 ) // audio sample with flat bit
        {
            if( msReadByte( REG_14EC ) & _BIT4 ) // error status bit
            {
                ...
                msWriteByte( REG_14EC, _BIT4 ); // write 1 to clear // 130530 JUN Modify
            }
            else
        ...
        {
            if( msReadByte( REG_14EF ) & _BIT2 ) // error status bit // 130530 JUN Modify
            {
