...
void msTMDSInit(void)
{
    ...
    msWriteByteMask(REG_29C9, 0xC0, 0xC0); // 121017 coding addition
...
void mStar_SetPanelSSC( BYTE freqMod, BYTE range ) //2011.9.13 18:33 CC
{
...
void drvDVI_PowerCtrl(DVI_PowerCtrlType ctrl)
{
    ...
    {
        // DVI power down
        msWriteByteMask(REG_PM_B2,  BIT2, BIT2); // power on DVI CKIN. (port 0 and port 1) (reg_b2[2])
        #if HDMI_PRETEST
        // Power up Xtal clocks and DVI detection clock for DVI clock detection
        msWrite2ByteMask(REG_290C, 0, 0xFFFF);
        // DVI PLL power control
        msWrite2ByteMask(REG_29C0, 0, 0xFFFF); // power down DVI0 PLL power
        #else
        // Power up Xtal clocks and DVI detection clock for DVI clock detection
    ...
    {
        // DVI power on
        msWriteByteMask(REG_PM_B2, 0, BIT2|BIT1|BIT0); // power on DVI CKIN. (port 0 and port 1) (reg_b2[2])
        msWrite2ByteMask(REG_290C, 0, 0xFFFF); // power on DVI power
        msWrite2ByteMask(REG_29C0, 0, 0xFFFF); // power on DVI PLL power
    }
}
...
void mStar_SetScalingFactor( void )
{
    ...
    width = msRead2Byte(SC0_0B)&SC_MASK_H;
    height = msRead2Byte(SC0_09)&SC_MASK_V;
    ...
    {
        Expansionwidth = (( DWORD )PanelHeight * width ) / ( height );
        Hstar = ( PANEL_WIDTH - Expansionwidth ) / 2 + PanelHStart;
        msWrite2Byte( SC0_18, Hstar );
        msWrite2Byte( SC0_1C, Hstar + Expansionwidth - 1 );
        #if Enable_Expansion
    ...
    {
        msWrite2Byte( SC0_18, PanelHStart );
        msWrite2Byte( SC0_1C, PanelHStart + PANEL_WIDTH - 1 );
        #if Enable_Expansion
    ...
    if( width != PANEL_WIDTH )        // set expansion factor
    {
...
{
    WORD u16PxlImgHDE = msRead2Byte( SC0_0B ); // image horizontal DE in pixel
    WORD u16PxlImgVDE = msRead2Byte( SC0_09 ); // image vertical   DE in pixel

    // wait for coding 
    WORD u16ImageOutH = PANEL_WIDTH;//GetImageOutH();
    WORD u16ImageOutV = PanelHeight;//GetImageOutV();
...
void I_Gen_Tuning( void )
{
    ...
    //TimeOutCounter = 50;
   SetTimOutConter(50);
   while (! ( msReadByte( REG_306E ) & BIT7 ) && bTimeOutCounterFlag);             // wait for MOD HW calibration done
...
void I_Gen_Tuning( void )
{
    ...
    for (i=0; i<=SWING_MASK; i++)   /*? if LVDS swing > 233mV , then REG_306C[7](C_DDA_OUT)=1*/
    {
		...
		if ( i==0 ||  i==SWING_MASK)
		{
			msWriteByteMask(REG_306F, SWING_MASK/2,SWING_MASK);						/* Set MOD output swing value for clock channel*/
...
Bool msDVIEQCalibration(void)
{
    ...
    msWriteByteMask(REG_29C9, 0x02, 0x02);// 121017 coding modified
    ...
    msWriteByteMask(REG_29C9, 0x00, 0x02); // 121017 coding modified
