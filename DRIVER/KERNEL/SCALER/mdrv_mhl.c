...
BYTE code MS_MHL_MDRVdummy[]={0xFF};

void MS_MHL_dummy()
{
    BYTE x = MS_MHL_MDRVdummy;
}
...
BYTE xdata g_u16S2PatchCntFlag = 0;
...
Bool mdrv_mhl_CableDetect(void)
{
    ...
    {  
        if(!GET_MHL_CABLE_PLUGGED())
        {
            ...
            //Set_MHL_PS_CTRL_CHARGE(); // 120720 coding test
#if ENABLE_MHL_COUNT
MHLActionCount = 1; // start to count
#endif
...
{
#if DMHL_CDR_SW_MODE    // SW mode
    mhal_mhl_CDRModeMonitor(GET_CBUS_LINK_FINISH());

#else    // HW mode
    mhal_mhl_GetTMDSClk();   // Solve uncall function warning
...
#if 0//ENABLE_MHL_COUNT
        printData("PATH_EN:%d", MHLActionCount);
        MHLActionCount = 1;
#endif
                    break;
