...
code msLoadTbl_S Mhl24bitsMode_JunoE_tbl[] = 
{
    ...
    //{REG_CHIPTOP_2E, BIT5, BIT5}, // [5]: reg_qfn40_mode(SI mode)    
    // switch to CDR mode
#if(DMHL_CDR_SW_MODE || DMHL_PLL_DETECT_HW_MODE)
...
code msLoadTbl_S MhlPowerSave_JunoE_tbl[] = 
{
    ...
    {REG_DVI_ATOP_6D, 0x0900, 0x0800}, // [11]: PD_BG, [10]: PD_RCK
#endif    
    //{REG_DVI_DTOP_27, BIT0, BIT0}, // [0]: MHL enable
    {REG_HDMITX_MISC_1E, 0x0006, 0}, // [2]: PD_TX_XTAL, [1]: PD_TX_ICLK
    {REG_HDMITX_MISC_2A, 0x000F, 0x000F}, // [3:0]: TMDS Tx power down
...
#if(MHL_DRIVER_VERSION >= MHL_DRIVER_U04)
...
{
    static WORD xdata wpre_val = 0;
    WORD xdata wcur_val;
    static BYTE xdata u8count = 1;
#if DMHL_CTS_HDCP_PATCH
    static BYTE xdata u8DEStableCnt = 0;
#endif
    BYTE xdata u8Threshold = 100;
...
#if ENABLE_MHL_COUNT
                printData("DE stable:%d", MHLActionCount);
                MHLActionCount = 1;
#endif
                }
    ...
    #if 0
    #if DMHL_CDR_SW_MODE
                //W2BYTEMSK(REG_DVI_DTOP3_0C_L, BIT(9)|BMASK(2:0), BIT(9)|BMASK(3:0)); //[9]: overwrite enable [3:0]: 4'h 0111
                W2BYTEMSK(REG_DVI_DTOP3_0E_L, 0, BIT(4)); // auto clear phase accumulator		
                W2BYTEMSK(REG_DVI_DTOP3_3A_L, 0x0A, 0xFFFF); // phase code = 0x0A
                W2BYTEMSK(REG_DVI_DTOP3_3B_L, BIT(4), BIT(4)); // overwirte enable
                W2BYTEMSK(REG_DVI_DTOP3_3C_L, 0x0A, 0xFFFF); // phase code = 0x0A
                W2BYTEMSK(REG_DVI_DTOP3_3D_L, BIT(4), BIT(4)); // overwirte enable
                W2BYTEMSK(REG_DVI_DTOP3_3E_L, 0x0A, 0xFFFF); // phase code = 0x0A
                W2BYTEMSK(REG_DVI_DTOP3_3F_L, BIT(4), BIT(4)); // overwirte enable
                W2BYTEMSK(REG_DVI_ATOP_5B_L, 0, BMASK(7:4)); // [7:4]: MHL_Ictrl = 0    
                W2BYTEMSK(REG_DVI_ATOP_62_L, BIT(12)|BIT(1), BIT(12)|BMASK(1:0));	 //[12]: cap, [1:0]: 2'b 10 sharp clock weakly
                W2BYTEMSK(REG_DVI_ATOP_64_L, BIT(9)|BIT(7), BIT(9)|BIT(7));	 // DVI DPL test register [45][9][7] = 1
                W2BYTEMSK(REG_DVI_ATOP_66_L, BIT(13), BIT(13));
    #else // DMHLPLLMODE
                W2BYTEMSK(REG_DVI_DTOP3_0C_L, BIT(9)|BMASK(2:0), BIT(9)|BMASK(3:0)); //[9]: overwrite enable [3:0]: 4'h 0111
                W2BYTEMSK(REG_DVI_ATOP_62_L, BIT(12)|BIT(1), BIT(12)|BMASK(1:0)); //[12]: cap, [1:0]: 2'b 10 sharp clock weakly
    #endif
