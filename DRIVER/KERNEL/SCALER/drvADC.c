...
#if (CHIP_ID == CHIP_TSUMD || CHIP_ID == CHIP_TSUM9 || CHIP_ID == CHIP_TSUMF ) 
...
#if (((_DGAIN_CAL_WITHOUT_INPUT_) && (DADCNEWGAINCALMETHOD == 0)) || (_DGAIN_CAL_WITH_INPUT_))
...
void drvADC_init(Bool IsShareGrd)
{
   ...
   // xdata BYTE no_use = 0;
    ...
    BYTE xdata  u8ADCBandgapTrimVal = 0;
#endif
...
void drvADC_AdjustHTotal(WORD u16HTotal)
{
	...
	msML_Trigger(ML_TRIG_OUT_VDE_END);// Vde-end-OP
        ...
        DB_WB(REG_ADC_DTOP_01_L, (u16HTotal-3));
        DB_WB(REG_ADC_DTOP_01_H, (u16HTotal-3)>>8);
...
Bool drvADC_AutoAdcColor(BYTE u8VSyncTime)
{
    ...
    //else
	 drvADC_SetRGBOffsetCode(0x800-OffsetValue, 0x800-OffsetValue, 0x800-OffsetValue);
...
void drvADC_ADCAutoGainCalwithoutInput(void)
{
    ...
    {
        case 0x00:
			UserPrefAdcRedGain = 0x514 + ADC_GAIN_FIX_LUM_0;
			UserPrefAdcGreenGain = 0x514 + ADC_GAIN_FIX_LUM_0;
			UserPrefAdcBlueGain = 0x514 + ADC_GAIN_FIX_LUM_0;
            break;
        case 0x15:
			UserPrefAdcRedGain = 0x5B7 + ADC_GAIN_FIX_LUM_1;
			UserPrefAdcGreenGain = 0x5B7 + ADC_GAIN_FIX_LUM_1;
			UserPrefAdcBlueGain = 0x5B7 + ADC_GAIN_FIX_LUM_1;
            break;
        case 0x2A:
			UserPrefAdcRedGain = 0x65A + ADC_GAIN_FIX_LUM_2;
			UserPrefAdcGreenGain = 0x65A + ADC_GAIN_FIX_LUM_2;
			UserPrefAdcBlueGain = 0x65A + ADC_GAIN_FIX_LUM_2;
            break;
        case 0x3F:
			UserPrefAdcRedGain = 0x6FC + ADC_GAIN_FIX_LUM_3;
			UserPrefAdcGreenGain = 0x6FC + ADC_GAIN_FIX_LUM_3;
			UserPrefAdcBlueGain = 0x6FC + ADC_GAIN_FIX_LUM_3;
            break;
        default:
            break;
    }
...
BYTE drvADC_WaitAutoStatusReady(WORD regAddr, BYTE regFlag)
{
    ...
    SetTimOutConter(50);
    while( bTimeOutCounterFlag && !( (u8Value=msReadByte( regAddr ))&regFlag ) );
#endif
...
static void drvADC_MismatchLinearityCal(void)
{
    ...
    do
    {
        ...
        // wait done
        ForceDelay1ms(1);
    } while( bTimeOutCounterFlag && ((drvADC_ReadByte(REG_ADC_DTOP_69_H) & 0x70) != 0x70) ); // [14:12]: linearity calibration done
    // Set Vcal 
...
#endif
#endif //#if _DGAIN_CAL_WITHOUT_INPUT_

#if ((_DGAIN_CAL_WITH_INPUT_) || ((_DGAIN_CAL_WITHOUT_INPUT_) && (DADCNEWGAINCALMETHOD == 0)))
//**************************************************************************
//  [Function Name]:
//                  drvADC_BackupScalerSetting()
//  [Description]:
...
#elif (CHIP_ID == CHIP_TSUMV || CHIP_ID == CHIP_TSUM2 || CHIP_ID == CHIP_TSUMC || CHIP_ID == CHIP_TSUMD || CHIP_ID == CHIP_TSUM9|| CHIP_ID == CHIP_TSUMF ) //
