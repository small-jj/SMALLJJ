...
//*******************************************************************
Bool mStar_SetupMode(void)																			//setup mode
{
#if ENABLE_DISPLAY_UNDERSCAN
    WORD hFreq, vFreq;
#endif

    Clr_FreeRunModeFlag();		//清空相关标志
...
// 121123 coding, moved here from mStar_PrepareForTimingChange()
	...
	#endif				//设置模拟信号的HTotal Phase Polarity等相关参数
...
#endif
    if (mStar_SetCaptureWindow() == FALSE)//通过获取输入信号的起始位置和尺寸大小来确定输出信号的起始位置和尺寸大小
    ...
    else
    {
		...
		mStar_SetupPath();						//对输出画面进行缩放的一些设置
#else
		mStar_SetScalingFactor(); // setup scaling factor//设置比例因子，通过比较输入和输出的width和height来设置横向和纵向的缩放因子
		mStar_SetScalingFilter();//按比例对水平和垂直方向进行缩放，无缩放比例 按比例增加 按比例缩小
#endif

    if (mStar_SetPanelTiming() == FALSE)//设置output dclk
    // set output dclk
    {
                   #if DEBUG_MSG
...
if(GetVfreq()>85
    ...
    if (InputTimingChangeFlag || mStar_ValidTimingDetect())//检测输入信号是否发生改变
...
#if UseINT
	mStar_EnableModeChangeINT(TRUE);//当输入信号改变时，使用中断加速模式改变
#endif


    mStar_InterlaceModeSetting();//设置隔行扫描
	...
	#endif
    mStar_SetUserPref(); // restore user setting 恢复用户设置，如：DCR、ColorTemp、Brightness等
    return TRUE;
}
//*******************************************************************
...
// callee: mStar_MonitorInputTiming() in detect.c
//*******************************************************************
void mStar_SetupInputPort(void)//数字端口需要拉下HPD
{
#if Dual
	if (!(FactoryModeFlag))
...
#endif
//CaptureWindow的原理是什么?
//原理：通过获取输入Image的起始位置和尺寸大小来设定输出Window的起始位置和尺寸大小,数字信号从寄存器中读取mode信息
    ...
    {	//从寄存器中读取mode信息
        UserPrefHStart = SC0_READ_AUTO_START_H();//msRead2Byte(SC0_80); // get hsync DE start 自动检测水平起点位置
        UserPrefVStart = SC0_READ_AUTO_START_V();//msRead2Byte(SC0_7E); // get vsync DE start 自动检测垂直起点位置
        //自动检测水平终点位置 - 自动检测水平起点位置 + 1
        width = SC0_READ_AUTO_WIDTH();//msRead2Byte(SC0_84)-msRead2Byte(SC0_80)+1; // get DE width
        //自动检测垂直终点位置 - 自动检测垂直起点位置 + 1
    ...
    printData("CAPTUREWIN UserPrefVStart001=%d",UserPrefVStart);
         ...
         if(CURRENT_SOURCE_IS_INTERLACE_MODE())
            ...
            if( PanelWidth == 1366 )		//120119 Modify
            {
                if( width == 1360 )
                {
                    UserPrefHStart -= 3;//1366的屏左右多出三个像素点的黑边
                    width = 1366;
                }
            }
