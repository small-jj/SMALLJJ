...
{	
	...
	if(ubAutoAlign==0)
	{
		if((DDCBuffer[3]==0x35)&&(DDCBuffer[4]==0x1f)&&(DDCBuffer[5]==0))//enter align modeËøõÂÖ•Ê†°ÂáÜÊ®°Âºè
		{
			Set_ATEModeFlag();
			Set_FactoryModeFlag();
			Set_BurninModeFlag();
...
//ÂèëÈÄÅÈ¢úËâ≤Êåá‰ª§
if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x13)&&(DDCBuffer[5]==0))//auto color//Ëá™Âä®Ë∞ÉÊï¥È¢úËâ≤	
{
		 if(AutoColor()!=0)
		 {
			DDCBuffer[1]=0x00;
...
#endif
//ËÆæÁΩÆ‰∫ÆÂ∫¶ÂíåÂØπÊØîÂ∫¶ÈªòËÆ§ÂÄº
...
//ËÆæÁΩÆÈªòËÆ§ËØ≠Ë®ÄÊàñÈÄâÊã©ÈáçÁΩÆËØ≠Ë®Ä
...
else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x23)&&(DDCBuffer[5]==1))//change source dvi
{
    ...
    else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x30))//3DDC≥§∂»
   {
	...
	else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x31))//3DDCΩ” ’
		{
			BYTE  j,i=0;
		   	j=DDCBuffer[5];
	 ...
	 else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x32))   //3ddC –£—È
		{
			 BYTE  checksun=0;
			 WORD  i;
	 ...
	 else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x33))//3ddc±£¥Ê
		{
			 BYTE  checksun;
			 BYTE  EDID_TYPE=0;
			   #if USEFLASH
		        ...
		        else
			  {
				...
				return 2;
			 		...
			 		Flash_Write_Factory_KeySet(FLASH_KEY_DDCD, FALSE,  DDCDKEY_SIZE, DDC1, DDCDKEY_SIZE);	
                        ...
                        checksun=DDC1[0x7F];  //2  ÷ª–Ë–£—È128 ◊÷Ω⁄
...
//ËÆ°ÁÆóÈïøÂ∫¶
    ...
    else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x30))//DDCÈïøÂ∫¶
		{
    			#if LoadAllPortSeriealnumberByOneVGAPort	//ÈÄöËøá‰∏Ä‰∏™ VGA Á´ØÂè£Âä†ËΩΩÊâÄÊúâÁ´ØÂè£Â∫èÂàóÂè∑		 
		   ...
		   edid_pointer=0;
			...
			{
				DDCBuffer[0]=0x84;//ÈïøÂ∫¶‰∏∫4
				DDCBuffer[1]=0x03;//3‰ª£Ë°®get
				DDCBuffer[2]=0x58;
				return 2;				
			}
		}

	else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x31))//DDCÊé•Êî∂
		{
			BYTE  j,i=0;
		   	j=DDCBuffer[5];
			if (edid_pointer<edid_cont)
	 ...
	 else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x32))   //2    ddC Ê†°È™å
		{
			 BYTE  checksun=0;
			 WORD  i;
			if ((edid_cont==128)||(edid_cont==256))
	 ...
	 else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x33))//DDC‰øùÂ≠ò
		{
			 BYTE  checksun;
			 BYTE  EDID_TYPE=0;
			   #if USEFLASH
...
#message "please check , dont  have this function"
#elif 0
	else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x34))//HDCP ≥§∂»
		{
			if((DDCBuffer[5])==6)
				{
				//µ±«∞≈˙¥Œ[6]
	...
	else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x35))//HDCP  Ω” ’
		{
			BYTE  j,i=0;
			j=DDCBuffer[5];
			if(DDCCI_pointer<DDCCI_cont)
				...
				if(DDCCI_pointer<=(DDCCI_cont-1))//2<=  ±£÷§ø®‘⁄◊Ó∫Û“ª¥Œ∑¢ÀÕ–£—È¬Î
					{
					...
					{
	//////////////////////////////÷¡¥À->±æ≈˙¥ŒΩ” ’ ˝æ›OK£¨ø…“‘“‘Hdcp_pointerŒ™OFFSET Ω¯––±£¥Ê/////////////////////////////////////
	...
	else if((DDCBuffer[3]==0x39)&&(DDCBuffer[4]==0x36))//HDCP –£—È
		{
		  WORD  wCount ; 
               BYTE   i=0;
		//‘⁄¥À¥¶∂‘±£¥Êµƒ ˝æ›Ω¯––∂¡»°£¨»Áπ˚–£—È∫Õ”Î¥À«∞Ω” ’µƒ–£—È∫Õœ‡µ»‘ÚOK,∑Ò‘ÚNG.
		 checksun_verify=0;
...
{
    if( rxStatus == DDC2B_COMPLETED )//Â¶ÇÊûúÊé•ÂèóÂÆåÊàê
    {
        DDC2Bi_ParseCommand();//Ëß£ÊûêÂëΩ‰ª§
        Clr_RxBusyFlag();//Ê∏ÖÈô§Êé•ÂèóÂøôÁ¢åÊ†áÂøó‰Ωç
        DDC2Bi_InitRx();//ÂàùÂßãÂåñ
    }
}
...
{//Âà§Êñ≠Á´ØÂè£Á±ªÂûã
    ...
    if( ADC_INT_FLAG &( BIT3 | BIT4 ) )//ADCÊï∞ÊçÆËØªÂÜô‰∏≠Êñ≠Ê†áÂøó‰Ωç
    {
        DoDDCCI( InputCombo_A0 );//DDC/CIÂ§ÑÁêÜÂáΩÊï∞
        CLR_ADC_INT_FLAG();//Ê∏ÖÁ©∫Ê†áÂøó‰Ωç
    }
  #if DDC_Port_D0
...
#endif
//Ëß£ÊûêÊï∞ÊçÆÂπ∂ÊâßË°åÁõ∏ÂÖ≥Êìç‰Ωú
#if Enabe_DDCCI_FUNCTION
...
BYTE AlignControl(void)
{
    ...
    BYTE  XDATA Resault;
 #endif
    CommandCode = DDCBuffer[1];//Âà§Êñ≠SET(0x03)ËøòÊòØGET(0x01)
    PageValue = 0;
	  ...
	  &&(MenuPageIndex != LogoMenu))
	{
	 ...
	 if(MenuPageIndex != InputInfoMenu)
     #endif	  	
	   {
		...
		MenuPageIndex=RootMenu;
	 	MenuItemIndex=0;	
 		Set_LoadOSDDataFlag();///ÈáçÊñ∞loadfontÁöÑflag
	   }
     	  }
	/*	Osd_Hide(); //20151211 for TPV Request
		MenuPageIndex=RootMenu;
    ...
    #endif //When DDCCI_ENABLE=0,we use below to disable DDCCI and Use it to adjust W/B
    {
        ...
        {
            CPCode = DDCBuffer[2];//Âà§Êñ≠ÈúÄË¶ÅÊâßË°åÁöÑÂä®‰ΩúÔºåÂ¶ÇCPCode=0x10‰∏∫‰∫ÆÂ∫¶Ë∞ÉËäÇ
            if (CPCode != 0xFE)
                return FALSE;
        }
        else
    ...
    {
        CPCode = DDCBuffer[2];//Âà§Êñ≠ÈúÄË¶ÅÊâßË°åÁöÑÂä®‰ΩúÔºåÂ¶ÇCPCode=0x10‰∏∫‰∫ÆÂ∫¶Ë∞ÉËäÇ
        RetValueH = DDCBuffer[3];//DATAÈ´ò‰Ωç
        RetValueL = DDCBuffer[4];//DATA‰Ωé‰Ωç
        WordValue = ((WORD)DDCBuffer[3] << 8) | ((WORD)DDCBuffer[4]);//ÊääÈ´ò‰ΩçÂíåÂú∞‰ΩçÁªÑÂêàÊàê‰∏Ä‰∏™WORDÂûãÊï∞ÊçÆ
...
//*******************************************************************************************
    ...
    //S-6Ea-51a-PktLena-Pre1Bytea-Pre2Bytea-CmdBytea-ExtBytea-DATA1a-DATA2a-CHKa-P
    	...
    	#ifdef UseVGACableReadWriteAllPortsEDID
	  #if 1  ///yuanzhi 20100428_1 for µ•ø⁄…’¬º≤ªÕ®Œ Ã‚.
    ...
    if (CommandCode == GETVCPFeature || CommandCode == GETPagedVCP)
    {
        ...
        {
            if(CPCode==TPV_App_Command)//Âà§Êñ≠CPCodeÂëΩ‰ª§ÊâßË°åÁõ∏Â∫îÁöÑÂä®‰Ωú
            	{
                DDCBuffer[0] = 0x88;
                DDCBuffer[1] = 0x02;
                DDCBuffer[2] = 0x00;
	...
	else if(CPCode == CleanUsedTime && PageValue == 0)
	{
        ...
        // Jeff add it in 2004¶~7§Î13§È§U§» 04:46:32
        {
            ValueH = 0xFF;
            ValueL = 0x16;
            RetValueH = 0;
...
/*******************************************************************************************************/
    ...
    else if (CommandCode == SETVCPFeature || CommandCode == SETPagedVCP)
    {
        ...
        else if (CPCode == ADJ_Brightness && PageValue == 0)//Âà§Êñ≠‰øÆÊîπ‰∫ÆÂ∫¶ÂÄº
        // 3-5
        {	
		#if CT_sRGB_ENABLE
