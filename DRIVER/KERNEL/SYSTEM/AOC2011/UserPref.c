...
void UserPref_FlashSaveFlagCheck(void)															//flash检测
{
...
//save mode 
void UserPref_FlashDataSaving(void)																	//flash更改
{
    ...
    {
        //USERP_printMsg("--UserPref_FlashDataSaving SaveModeSetting");											//save mode  my
...
{
	if (SrcInputType>=Input_Digital || SyncLossState())	//110424 Modify//数字信号和不稳定信号不保存
		return;
   ...
   // USERP_printData("UserModeFlag %x",UserModeFlag);
	...
	if(UserModeFlag)
	{	ModeSetting.Index = SaveIndex|BIT7;//|= 0x80;   //Index从0x80开始
	...
	ModeSetting.Index &= 0x7F;					//还原为非UserMode
	#if 0//Enable_Cache
	MCU_EnableCache();
	#endif
...
void ReadMonitorSetting(void)
{
	...
	Flash_GetBufferAddr();//空间标识获取各类buffer在Flash中的地址
		Flash_AllocateBufferAddr();//检查HDCP_EDID/Factory/MonitorSetting/MonitorSetting2/ModeSetting中是否有未分配到Block，没有则重新分，确保合法
		if(IsFlashSettingAreaEmpty(0x00)||IsFlashSettingAreaEmpty(0xFF))//edit by lizzie.ding ,for make sure flashsetting empty
      	 {//校验特定缓冲区的地址是否合法
		FlashEmptyFlag=1;//如果闪存里面动态块是空的，置起FlashEmptyFlag标志
      	 }
		Flash_CheckBufferType();//校验特定缓冲区类型是否正确，错误擦除缓冲区，再写正确的
	#if Display_Checksum
            Checksum_Calculator();
	#endif	
		if(!Flash_ReadMonitorSet((BYTE*)&MonitorSetting, MonitorSettingSize))//判断是否读的到屏幕设置，读不到重新初始化并保存
		{
			//printMsg(" RMS MonitorSetting initial");
			Init_MonitorSetting();//重新初始化赋值
			if(FlashEmptyFlag)
				{
		...
		if (CheckMonitorSettingOutOfRange())//如果数据超出范围
		{
			//USERP_printMsg(" CheckMonitorSettingOutOfRange initial");
			Init_MonitorSetting();//重新初始化赋值
			SaveMonitorSetting();//将monitor类型的数据写到flash中
		}
	
		if(!Flash_ReadFactorySet((BYTE*)&FactorySetting, FactorySettingSize))//检查factoryset是否正确
		{
		...
		if(!Flash_ReadMonitorSet2((BYTE*)&MonitorSetting2, MonitorSetting2Size))//判断是否读得到背光值的设置，读不到就初始化并保存
		{
...
Bool ReadUserModeSetting(void)
{
	...
	hFreq=HFreq(SrcHPeriod);//HFreq(tempHperiod);//计算hfreq
	vFreq=VFreq(hFreq, SrcVTotal);//计算vfreq

	// 20070426Albert.Search each user mode
	for (i=0; i<UserModeNumber; i++)//查找UserMode，共5组
    {
    	Flash_ReadModeSet((i|BIT7), (BYTE*)&ModeSetting);	// BIT7 == (UserModeFlag<<1)//Index从0x80开始
		if(ModeSetting.Ident==FLASH_IDENTDATA && ModeSetting.checksum==FLASH_CHECKSUM)//校验ModeSetting的头和尾
		{	if (abs(hFreq-ModeSetting.HerFreq)<HFreq_Tol &&    //16
		abs(vFreq-ModeSetting.VerFreq)<VFreq_Tol &&				//8
				(SrcFlags&0x13)==ModeSetting.Flags)//bit4:隔行扫描 bit0 bit1:极性
				break;
		}
    }
...
void ReadModeSetting(void)
{
	...
	if (bFound)//未找到，在BLOCK区查找没有发现相应的mode 185次
	{
	...
	else
	{
		...
		Init_ModeSetting();//初始化，将默认数据导入
		SaveModeSetting();//保存
	}
	CheckModeSettingRange();//校验ModeSetting范围
}
