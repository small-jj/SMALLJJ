...
void appCheckSimilarTiming(void)
{
    ...
    if(ucSimilarTimingNums <= 1 || (SrcFlags&bUnsupportMode))////如果相似Mode 个数小于等于1或者SrcFlags为UnsupportMode
        return;

    vsyncTime = GetVSyncTime();//退出当前函数
    i = 0;
    maxcksum = maxcksum1 = 0;
	
    ucIndex = SrcModeIndex=ucSimilarTimingBuffer[0];	///  取ucSimilarTimingBuffer 数组中的第一个mode
    ...
    while(uccount--)
    {
        ...
        wWidth = HFreq( SrcHPeriod );//计算HFreq
        wWidth = (( DWORD )wWidth * StandardModeHTotal + 5000 ) / 10000; //dclk= hfreq * htotal//计算DCLK = HFreq * HTotal
        /// SOG信号，则为YPBPR格式，否则为RGB格式 带 PLL 保护的设置模式
...
#if CHECK_MODE_WIDTH
        wWidth = SearchMaxWidth(vsyncTime);//获取最大宽度
        
        //printData(" width:%d", wWidth);
            
        if(abs(wWidth - StandardModeWidth) < 2)     //111223 Modify // 如果最大带宽与mode表中mode 的带宽差值绝对值小于2
        {
        ...
        SC0_SET_IMAGE_WIDTH(StandardModeWidth);//msWrite2Byte( SC0_0B, StandardModeWidth);表中mode的dispWidth
        SC0_SET_IMAGE_HEIGHT(StandardModeHeight);//msWrite2Byte( SC0_09, StandardModeHeight );表中的disHeight

        for( adjPhase = 0; adjPhase < PHASE_MAX; adjPhase += PHASE_STEP)//PHASE_MAX = 0x80  PHASE_STEP = 0x07  18次
        {
            drvADC_SetPhaseCode( adjPhase );//设置ADC相位码
            Delay1ms( vsyncTime + 3 );
            AUTO_PHASE_RESULT_READY();//WaitAutoStatusReady(SC0_8B, BIT1);//等待自动相位结果准备就绪
            //cksum = msRead2Byte(SC0_8E);
            //cksum = ( cksum << 16 ) | msRead2Byte(SC0_8C);
            cksum = AUTO_PHASE_READ_VALUE();//自动读取相位值
            if( cksum > maxcksum )
            {
                maxcksum = cksum;
            }
        ...
        if(maxcksum > maxcksum1)//判断每个mode的最大Phase点，如果是正确的mode，Phase应该为最大
        {
            maxcksum1 = maxcksum;
...
#define fStatus  hFreq
    ...
    fStatus = mStar_GetInputStatus();//获取输入信号的状态和极性
    if( SyncPolarity( SrcFlags ) != SyncPolarity( fStatus ) )        // Sync polarity changed 和侦探到的信号状态比较，判断信号极性是否改变
    {
        return FALSE;
    }
...
#undef fStaus
#if ENABLE_FREESYNC
    if (!IS_DP_FREESYNC())//如果不是数字信号，进
#endif
    {
    hFreq = SC0_READ_HPEROID();//msRead2Byte(SC0_E4) & 0x1FFF;
    if( abs( hFreq - SrcHPeriod ) > HPeriod_Torlance )  // HPeriod changed与上一次的HPeriod进行判断，看差值是否大于10，来检测信号是否发生改变
    {
        return FALSE;
    }
    }
#if ENABLE_FREESYNC
    if (!IS_HDMI_FREESYNC())//如果不是数字信号，进
#endif
    {
      vFreq = SC0_READ_VTOTAL();//msRead2Byte(SC0_E2) & 0xFFF;
      if( abs( vFreq - SrcVTotal ) > VTotal_Torlance )  // vtotal changed与上一次的VTotal进行判断，看差值是否大于4，来检测信号是否发生改变
      {
          return FALSE;
      }
    }
...
#endif	
																			////SetPanelTiming做了哪些事情？	
	//111223 Modify	
    if( SrcHPeriod > 511 )        
    {
        hFreq = SrcHPeriod * 16;
    }
    else										//当输入频率很低时，防止计数器溢出
    {
        tempHeight = GetVSyncTime() * 12; ///3	//120112 Modify
        SC0_HPEROID_DETECT_MODE(TRUE);//msWriteByte(SC0_E5, BIT7); // 启用 16 行计算同步周期.	//120105 Modify
        Delay1ms( tempHeight );
        hFreq = SC0_READ_HPEROID();		//读取输入信号的Hperiod					//120105 Modify
	...
	/////////////////////////////////////////////////////////
	SrcHPeriod = hFreq;								//将读取并计算得到的hFreq赋值给SrcHPeriod
    SrcVTotal = vFreq;								//将读到的vFreq赋值给SrcVTotal
	...
	hFreq = HFreq( SrcHPeriod );									//计算hFreq
	vFreq = VFreq( hFreq, SrcVTotal );								//计算vFreq
    }
#if DEBUG_PRINTDATA
  ...
  if(SrcFlags&bInterlaceMode)
  {
...
#else																	
        if( (SC0_READ_SYNC_STATUS()&INTM_B) //访问不同的寄存器来检测不同的信号是否开启隔行扫描，该芯片的隔行/非隔行检测结果。0: 非隔行。1:隔行
#if ENABLE_DP_INPUT
	...
	)
	{
		...
		{
			BYTE count;
			for( count=0; count<RECHECK_INTERLACE_COUNT; count++ )//模式轮询扫描25次，期间有任意一次表明不需要开启隔行扫描，则关闭隔行扫描
			{
			...
			if(count>=RECHECK_INTERLACE_COUNT)//25次后若正常结束循环则开启隔行扫描
			{
	...
	//检查输入信号的场频和行频是否超出了范围 MaxInputHFreq:900 MinInputHFreq:140 MaxInputVFreq:800 MinInputVFreq:490
	...
	#if ENABLE_FREESYNC
	#else
	    SrcFlags |= bUnsupportMode;	//信号不支持											//my
	    #if DEBUG_MSG
		printData("hFreq %d", hFreq);
		printData("vFreq %d", vFreq);
	...
	htol = HFreq_Torlance; //0527 add for getting more accurate mode//添加以获得更精确的模式
	vtol = VFreq_Torlance;

	ucSimilarTimingNums = 0;
	SrcModeIndex = 0xFF;

    if(!CURRENT_INPUT_IS_VGA())//(SrcInputType >= Input_Digital)//判断输入信号是否为数字信号
    {
        bDigitalInput = 1;//数字输入
#if ENABLE_DP_INPUT     //111027 Rick modified - B39435
        if(CURRENT_INPUT_IS_DISPLAYPORT())//(SrcInputType == Input_Displayport)当前输入是显示端口
        {
    ...
    // search input mode index
    {
        ...
        for(ucModeIndex = ucStartIndex; modePtr->HFreq > 0; modePtr++, ucModeIndex++)//查找mode表，从mode表中的第一个元素开始查找
        {
		if(bDigitalInput)//若为数字信号，将读到的Width Height与当前找到的mode的DispWidth DispHeight比较，判断其差值是否大于误差 3
		{							//标准模式分辨率
        		if((abs(tempWidth - StandardModeResolution[modePtr->ResIndex].DispWidth) > DIGITAL_TOLERANCE) ||
        			...
        			continue;
        																...
        																//间隔模式   bInterlaceMode = bit4
            if((CURRENT_SOURCE_IS_INTERLACE_MODE()) != (modePtr->Flags & bInterlaceMode))//判断当前找到的mode是否是隔行扫描信号
                continue;

            if(SrcVTotal < StandardModeResolution[modePtr->ResIndex].DispHeight)//判断SrcVTotal是否小于当前找到的mode的DispHeight
                continue;
			//将当前输入信号的hFreq、vFreq、SrcVTotal参数与当前找到的mode表中的hFreq、vFreq、VTotal相比较
			//判断hFreq vFreq VTotal是否在误差范围内
            if((abs(hFreq - modePtr->HFreq) < HFreq_Torlance) && 
                ...
                (GetSyncPolarity( SrcFlags )&modePtr->Flags))//比较极性
            {	
            //将hFreq vFreq和SrcVTotal与找到的mode中的差值分别赋值给ucHVtolerance ucVtotal
                ...
                {
                    found = TRUE;
                    ucHVtolerance_min = ucHVtolerance;//将当前差值赋值给最小误差
                    ucVtotal_min = ucVtotal;
                    SrcModeIndex = ucModeIndex;//将临时变量ucModeIndex赋给全局变量SrcModeIndex，即查找到当前mode在mode表中的位置     
					
                    ucSimilarTimingBuffer[ucSimilarindex] = ucModeIndex;////将当前索引ucModeIndex存放在相似Timing数组中的第0个位置
                    ucSimilarTimingNums ++;//分辨率相似的个数++
                    ucSimilarindex = (ucSimilarindex+1)%SIMILAR_TIMING_BUFFER_MAX;//下次进来时存放在下一个位置
					//将找到的mode存入信号相似表，并将范围值缩小，继续查找，只有下一次查找到的比上一次的更接近才会去更新信号相似表，
					//该表最多存10个，存满10个之后若第11个满足要求则会把表中的第一个覆盖掉
        ...
        if( !found )
        {
            modePtr = StandardMode;	//150                    100
            ucHVtolerance_min = HFREQ_TOLERANCE_ENLARGE + VFREQ_TOLERANCE_ENLARGE;				//最小容差增大
            ucVtotal_min = VTotal_Delta;	//50													//最小容差增大
            ...
            for(ucModeIndex = ucStartIndex; modePtr->HFreq > 0; modePtr++, ucModeIndex++)//查找mode表，从mode表中的第一个元素开始查找
            {
                if(bDigitalInput)
                {
                //若为数字信号，将读到的Width Height与当前找到的mode的DispWidth DispHeight比较，判断其差值是否大于误差 3
                    if((abs(tempWidth - StandardModeResolution[modePtr->ResIndex].DispWidth) > DIGITAL_TOLERANCE) ||
                        (abs(tempHeight - StandardModeResolution[modePtr->ResIndex].DispHeight) > DIGITAL_TOLERANCE))
                        continue;
                }
                ...
                if(SrcVTotal < StandardModeResolution[modePtr->ResIndex].DispHeight)//判断SrcVTotal是否小于当前找到的mode的DispHeight
                    continue;
				//扩大误差范围查找，比较hFreq vFreq SrcVTotal
				//将当前输入信号的hFreq、vFreq、SrcVTotal参数与当前找到的mode表中的hFreq、vFreq、VTotal相比较
				//判断hFreq vFreq VTotal是否在误差范围内
                if((abs(hFreq - modePtr->HFreq) < HFREQ_TOLERANCE_ENLARGE) && //150
                    (abs(vFreq - modePtr->VFreq) < VFREQ_TOLERANCE_ENLARGE) && //100
                    (abs(SrcVTotal - modePtr->VTotal) < VTotal_Delta))//50
                {
                //将hFreq vFreq和SrcVTotal与找到的mode中的差值分别赋值给ucHVtolerance ucVtotal
                    ucHVtolerance = abs(hFreq - modePtr->HFreq) + abs(vFreq - modePtr->VFreq);
                    ucVtotal = abs(SrcVTotal - modePtr->VTotal);
        ...
        if( !found )            // out of standard input range
        {
            ...
            for(ucModeIndex = ucStartIndex; modePtr->HFreq > 0; modePtr++, ucModeIndex++)//查找mode表，从mode表中的第一个元素开始查找
            {
                ...
                if(abs(SrcVTotal - modePtr->VTotal) < VTotal_Delta)//50
                {
                    ucVtotal = abs(SrcVTotal - modePtr->VTotal);
                    ...
                    {
                        found = TRUE;
                        SrcFlags |= bUserMode;   //将SrcFlags置为bUserMode
                        ucVtotal_min = ucVtotal;//将当前差值赋值给最小误差
                        SrcModeIndex = ucModeIndex;//将临时变量ucModeIndex赋给全局变量SrcModeIndex，即查找到当前mode在mode表中的位置
                        ucSimilarTimingBuffer[ucSimilarindex] = ucModeIndex;///将当前索引ucModeIndex存放在相似Timing数组中的第0个位置
                        ucSimilarTimingNums ++;//分辨率相似的个数++
                        ucSimilarindex = (ucSimilarindex+1)%SIMILAR_TIMING_BUFFER_MAX;//下次进来时存放在下一个位置
		...
		if(CURRENT_INPUT_IS_VGA())	//120420 Modify									//my 三次后
	    		appCheckSimilarTiming();					//找完所有的mode会进入该函数进一步判断区分相似的Timing
		#if DEBUG_PRINTDATA
...
if(SrcModeIndex==MODE_1280x800_60Hz_VESA||SrcModeIndex==MODE_1280X768_60Hz_VESA_RB)
	{
		...
		//有的mode通过上面的比较仍然无法区别，可以通过SrcVTotal、VSyncWidth继续比较
		//找不到mode且为VGA信号下，
	    if( !found&&(CURRENT_INPUT_IS_VGA()))
	    {
	        ...
	        #endif
	        SrcFlags |= bUnsupportMode;			//将标志置为不支持
	    }
            else   // found
    	    {
#if ENABLE_DEBUG
