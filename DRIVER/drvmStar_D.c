...
#ifndef _DRVMSTAR_D_C_//130604 Modify
...
{
#if ENABLE_AUDIO_AUTO_MUTE // 130530 JUN Modify
    ...
    {REG_01CA, 0x10F8, 0xFFFF},//130604 Modify
...
code RegUnitType tblInit[]=
{
  ...
  {SC0_15, ( PANEL_HEIGHT - 1 ) >> 8}, // Vert. DE end
    ...
    {REG_3816, 0x43},
    {REG_3817, 0x43},
    //{REG_3818, 0x08}, //nick remove 20130517 add auto color  //
...
void msInitClockGating( void )
{
    ...
    msWriteByteMask(REG_1E46,0x00, 0x0F);   // enable MRW clock 	//20130909 nick modify
}
...
#if (ENABLE_TNR)||(ENABLE_RTE)||(FRAME_BFF_SEL == FRAME_BUFFER)
    msInitMemory();
#endif
...
void mStar_InterlaceModeSetting( void )
    {
        ...
        if( CURRENT_SOURCE_IS_INTERLACE_MODE() )
        {
            ...
            else            //110921 Rick modified - B011
            {
                ...
                {
        #if ENABLE_DP_INPUT 	//130912 nick
...
void mStar_SetDClkPLL(DWORD u32ODCLK_KHZ)	//130909 nick
{
...
#if FRAME_BFF_SEL==FRAME_BUFFER  // nick check FB mode is correct or not
...
#endif
    {
        msWriteByteMask(SC0_27,BIT6,BIT6|BIT1);//20130517 add auto color
        msWrite2ByteMask(SC0_1E, PanelVTotal-1,SC_MASK_V); // set output panel vtotal
        msWrite2Byte(SC0_24, PanelHTotal-1);// 20130517 msWrite2Byte(SC0_24, (PanelMinHTotal+8)-1);
        msWriteByteMask(SC0_63,BIT0,BIT1|BIT0);
        msWriteByteMask(SC0_28,BIT6,BIT6);
        msWriteByteMask(SC0_28,BIT3,BIT3);//20130517 msWriteByteMask(SC0_28,0,BIT3);
        //msWriteByteMask(SC0_27,BIT6|BIT1,BIT6|BIT1);//20130517
...
void mStar_SetTimingGen( WORD u16HDE_OP21, WORD u16VDE_OP21, BOOL OP21_FreeRun)//20130909 nick modify
{
    WORD uwV_total_OP22; // output vtotal OP2-2
    WORD uwV_total_OP21; // output vtotal OP2-1

    drvmStar_printMsg(" 33333 mStar_SetTimingGen");
    ...
    {
        #if ENABLE_SUPERRESOLUTION // 20130530 nick modify for Clear version SRAM error 
            msWrite2Byte(SC0_10, 0x02); //panel V. start
    ...
    else // Case 3456, FB free run
    {
        #if ENABLE_SUPERRESOLUTION // 20130530 nick modify for Clear version SRAM error 
            msWrite2Byte(SC0_10, 0x02); //panel V. start
            msWrite2ByteMask(SC0_14, ( PanelHeight + 1 ), 0x7FF); //panel Vert. DE end
        #else
...
BYTE mStar_ClassifyVfreqBefCalFRCDivider( BYTE u8Vinfreq ) 	//20130909 nick modify
{
...
BYTE mStar_SetFrameDivider( BYTE vfreq_N )		//130724 nick
{
    ////////////////////////////////
...
BYTE GetVfreq( void )	//130909 nick
{
...
{
    msWriteByteMask(SC0_03, 0, _BIT7);                   //DP YUV422 Issu
    msWriteByteMask( REG_06C2, BIT2|BIT3, BIT2|BIT3 );                   //DP Port B/C Off Line
    msWriteByteMask(SC0_A8, 0x00, _BIT1|BIT0);
#if (HDMI_RB_SWAP)
    msWriteByteMask(REG_1600,_BIT5,_BIT5);
...
WORD GetImageOutH()
{
    ...
    if (EXPANSION_MODE_IS_FULL() || INPUT_IS_WILD_TIMING())//(UserprefExpansionMode==Expansion_Full)	//130916 xiandi
        return PANEL_WIDTH;
    ...
    #endif
    if (!EXPANSION_MODE_IS_FULL() && !INPUT_IS_WILD_TIMING())//(UserprefExpansionMode==Expansion_Aspect)	//130916 xiandi
    {
        u8HRatio=4;
        u8VRatio=3;
    }
...
WORD GetImageOutV() 	//20130909 nick modify
{
    return PANEL_HEIGHT;
}
#endif
...
Bool msPM_StartRCOSCCal(void)
{
#if (CHIP_ID== CHIP_TSUMC) || (CHIP_ID == CHIP_TSUMD)|| (CHIP_ID == CHIP_TSUMD)
