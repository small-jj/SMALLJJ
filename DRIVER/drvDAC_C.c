...
BOOL xdata g_bInternalI2S48K = FALSE;

#if ENABLE_I2S_ALWAYS_OUT
void msAudioEnableI2SInternal48K ( BOOL bControl, BYTE ucDebug );
#endif
...
void msAudioDAC_Init( void )
{
    ...
    // DAC audio clock
    msWriteByteMask(REG_1EE0, BIT4, BIT5|BIT4);  //130624 nick add for Check list request
...
#if ENABLE_I2S_ALWAYS_OUT
 msAudioEnableI2SInternal48K ( TRUE, 0 );
#endif
...
#define DEF_EARPHONE_VOLUME_H 0x7F0    // 0x700

#define DEF_EARPHONE_VOLUME_L      0x00
#define MIN_EARPHONE_VOLUME      0xA0
#define DefalutVolume			90

void msAudioDPGA_SetVolume( AUDIO_CH_SEL chsel, int volume ) //need check
{
      WORD VolumeValue =volume ; 
  /*    if ( volume > DefalutVolume )
        {
            VolumeValue = MAX_EARPHONE_VOLUME - ( ( DWORD ) ( MAX_EARPHONE_VOLUME - DEF_EARPHONE_VOLUME_H ) * ( volume - DefalutVolume ) ) / (100-DefalutVolume);
        }
        else
        {
            VolumeValue = DEF_EARPHONE_VOLUME_L + ( ( DWORD ) ( MIN_EARPHONE_VOLUME - 0 ) * ( DefalutVolume-volume ) ) / (DefalutVolume);
        }
*/
    switch(chsel)
    {
        case E_AUDIO_LEFT_CH:
            msWriteByte(REG_2CA6, (VolumeValue&0xFF));
            msWriteByte(REG_2CA7, ((VolumeValue>>8)&0x07));
            break;

        case E_AUDIO_RIGHT_CH :
            msWriteByte(REG_2CA8, (VolumeValue&0xFF));
            msWriteByte(REG_2CA9, ((VolumeValue>>8)&0x07));
            break;
...
{
    //[14:12] REG_CH_SEL      (Mux¡A¿ïHDMI,Linein or DMA(No-used,Test only)¡^
...
void msAudioLineout_SourceSel( AUDIO_LINEOUT_SOURCE_SEL src )
{
    ...
    #if 0//MainBoardType == MainBoard_7452_M0A  //|| MainBoardType == MainBoard_7762_M0A
    if(src==E_LINEOUT_FROM_LINEIN0)
    {
        if(!PHONE_DET)
            msWriteByteMask(REG_2C2D, BIT1 , BIT1);
        else
            msWriteByteMask(REG_2C2D, 0 , BIT1);
    }
    else
       msWriteByteMask(REG_2C2D, 0 , BIT1);
    #endif
...
{
	msAudioDPGA_SetVolume(E_AUDIO_LEFT_RIGHT_CH,val);
}
...
void msAudioGainForceToggle(void)  //130529 Nick add toggle audio gain to prevent zero crossing may not work in small signal 
{
    msWriteByteMask(REG_2CA4, 0, BIT7);
    msWriteByteMask(REG_2CA4, BIT7, BIT7);
    msWriteByteMask(REG_2CA4, 0, BIT7);
...
void msAudioEnableI2SInternal48K ( BOOL bControl, BYTE ucDebug )
{
    ...
    if ( bControl )
    {
        g_bInternalI2S48K = TRUE;
        msWriteByteMask ( REG_1770, BIT6, BIT6 );
    }
    else
    {
        g_bInternalI2S48K = FALSE;
        msWriteByteMask ( REG_1770, 0x00, BIT6 );
    }

#if ENABLE_DEBUG
    printData ( "\r\n[I2SInternal48K=%d", bControl );
    printData ( ", %d]", ucDebug );
#endif
}
#endif
