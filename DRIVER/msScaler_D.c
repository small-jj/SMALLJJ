...
static void mStar_SetVScalingFilter( BYTE u8Filter )
{
    ...
    if(( u8Filter & 0x0F ) == 0 )
    {
        ...
        #if 1//(PanelType != PanelLTM270HP01 && MainBoardType != MainBoard_7452_M0A&&MainBoardType != MainBoard_7762_M0A) //for choram 2233-B Pattern 2054 on Issue
        if (CURRENT_INPUT_IS_TMDS()&&((StandardModeGroup==Res_1920x1080)||(StandardModeGroup==Res_1920x1200)))	// 130723 nick  For BrightFrame on Issue
            {
                DB_WB( SC0_57 , 0x20 );
                DB_WBMask( SC0_5A,  1, ( BIT1 | BIT0 ) ); /* Vertical scaling filter is Bilinear mode */
            }
        #endif
    }
    else
...
void mStar_SetScalingFactor( void )
{
    ...
    if(!EXPANSION_MODE_IS_FULL() && !INPUT_IS_WILD_TIMING())//( UserprefExpansionMode == Expansion_Aspect && ExpansionFlag )
    {
        if(((g_SetupPathInfo.ucSCFmtOut == SC_FMT_OUT_3D_SBS)||(g_SetupPathInfo.ucSCFmtOut == SC_FMT_OUT_3D_CB))
            ...
            Expansionwidth = GetImageOutH()/2;
        else
            Expansionwidth = GetImageOutH();
	#if ENABLE_OVER_SCAN
		if(OverScanSetting.Enable && (g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL)
		 && ((EXPANSION_MODE_IS_11()||IS_ScreenSizeMode)&&!(INPUT_IS_NATIVE_TIMING()||(StandardModeWidth > PANEL_WIDTH) || (StandardModeHeight > PANEL_HEIGHT))))
		{
            Expansionwidth = g_SetupPathInfo.wImgSizeOutH;
		}
	#endif
...
static void msSetupPathParam() 	//20130909 nick modify
{
    BYTE ucMRWOut3DFmt;
    WORD u16VFreq;	//130529 Nick
    u16VFreq = VFreq ( HFreq ( SrcHPeriod ), SrcVTotal ) + 5;	//130529 Nick
    if(CURRENT_SOURCE_IS_INTERLACE_MODE())	//130608 xiandi
    {
        u16VFreq *= 2;
    }
    ...
    if( !EXPANSION_MODE_IS_FULL() && (g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL) && !INPUT_IS_WILD_TIMING())//( (UserprefExpansionMode!=Expansion_Full) && (g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL) ) // 130530 JUN Modify
    {
        if(((g_SetupPathInfo.ucSCFmtOut == SC_FMT_OUT_3D_SBS)||(g_SetupPathInfo.ucSCFmtOut == SC_FMT_OUT_3D_CB))
        &&(g_SetupPathInfo.ucSCFmtIn != SC_FMT_IN_NORMAL) && (!g_SetupPathInfo.b3DTo2DEnable))
            g_SetupPathInfo.wImgSizeOutH = GetImageOutH() / 2;
        else
        ...
        || g_SetupPathInfo.b2DTo3DEnable )
            g_SetupPathInfo.wImgSizeOutV = GetImageOutV() / 2;
        else
...
static void msSetupPathVSP()
{
    ...
    if( g_SetupPathInfo.bFBMode && g_SetupPathInfo.bScalingUpV &&CURRENT_SOURCE_IS_INTERLACE_MODE())	//130608 xiandi
