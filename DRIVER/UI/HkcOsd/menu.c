...
Bool DrawTimingInfo(void)
{
	...
	Osd_DynamicLoadFont(ResoulationAddress, ResolutionProp, 27); //20180109  –ﬁ∏ƒ∑÷±Ê¬ «–ªªµΩ1152x864 60hz ª·≥ˆœ÷¬“¬Î
...
void DrawOsdBackGround(void)
{
	...
	for(i = 3; i <= CurrentMenu.YSize - 8;i++)// ÁîªÁ´ñÁ∫ø
	{
		Osd_DrawCharDirect(0x16,i,0x0B);	//x,yËΩ¥ÔºåÁ¨¨‰∏âÂèÇÊï∞‰∏∫Á´ñÁ∫øÂú∞ÂùÄ
	}
		for(i = 3; i <= CurrentMenu.YSize - 8;i++)
	{
		Osd_DrawCharDirect(0x29,i,0x0B);
	}
		...
		{																   
			for(j=0;j < 2;j++)
			{
				...
				Osd_Set256TextColor((Pallef[i][j]>>2), Color_4); 
				Osd_DrawCharDirect(j, i, strlefWindow[i][j]-0x200);	
				OSD_FONT_HI_ADDR_CLR_TO_0();	
			}
		}
