...
WORD mSTar_GetInputHTotal(void)	//130730 modify
{
    WORD u16HTotal;
#if ENABLE_DP_INPUT
...
Bool appmStar_SetPanelTiming( void )
{
    ...
    {
        #if UseINT && Enable_Expansion //20130909 nick modify
    ...
    //***************************************************************
    if( sclk && !g_SetupPathInfo.bFBMode) //20130909 nick modify
    {
        ...
        if(dclk > PanelMaxDCLK)// && !OverScanFlag)
        {
            ...
            if(SrcVTotal-height > 12)
            {
                ...
                DecVScaleValue = ((VSTART_OFFSET>=SC0_READ_AUTO_START_V())?(SC0_READ_AUTO_START_V()*2):(DecVScaleValue));
       		  Set_DecVScaleFlag();//20130517 add
            }
            else
            {
                 Clr_DecVScaleFlag();//20130517 add
...
//****************Setup line buffer clock*************************************
    ...
    if(g_SetupPathInfo.bFBMode) //20130909 nick modify
    {
    ...
    mStar_SetTimingGen(PANEL_WIDTH, PANEL_HEIGHT, FALSE); 	//20130909 nick modify
    ...
    if(g_SetupPathInfo.bFBMode && INPUT_TIMING_EXECUTE_FRAME_PLL_LOCK())  //20130909 nick modify
    {
