...
void drv_EnableCTSNFileterFunction( void )
{
    ...
    msWrite2Byte( REG_140E, 0x0A0A ); //[15:8]: N range, [7:0]: CTS range
...
#define Repair_Clock_CTSNRangeFail                   1
#if Repair_Clock_CTSNRangeFail
BYTE GetAudioMuteStatus( void )
{
	BYTE bVolume_notmuteFlag  = 0;
	if((msReadByte(REG_2CA1) & (BIT3|BIT2)) != (BIT3|BIT2) ) // no mute  ,
	    bVolume_notmuteFlag = 1;
	
	return bVolume_notmuteFlag;
}
...
#if Repair_Clock_CTSNRangeFail
		   bVolume_notmuteFlag  = GetAudioMuteStatus();
	         if(bVolume_notmuteFlag)
	         	{
			 msWriteByteMask(REG_2CA1, BIT3|BIT2 , BIT3|BIT2);
			 }
	         ...
	         if(bVolume_notmuteFlag)
	         	{
	         	 Delay1ms(5);
			 msWriteByteMask(REG_2CA1, 0 , BIT3|BIT2);
			 }
