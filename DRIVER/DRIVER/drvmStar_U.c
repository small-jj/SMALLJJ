...
#define DRVMSTAR_DBG     0

#if DRVMSTAR_DBG && ENABLE_DEBUG
#define drvmStar_printMsg(str)        printMsg(str)
#define DRVMSTAR_printData(str, x)     printData(str, x)
#else
#define drvmStar_printMsg(str)
#define DRVMSTAR_printData(str, x)
#endif


#if 0	//120112 Modify
extern void appmStar_InitPanelPara(void);
#endif
code RegUnitType tblInit[]=
{
    ...
    {SC6_03, 0x40},
    {SC6_BE, 0x03}, // interlace shift line setting // 121101 coding addition
  #endif
...
#if 0//(PanelType==PanelSAM27_LTM270DL02_QHD_60HZ) //20130621 nick
      {SC0_36, 0x01},
#else
    #if PanelDither==6
...
void msInitClockGating( void )
{
    ...
    #if 0//((ENABLE_RTE) && (OD_MODE == OD_MODE_333) && !OD_MODE_333_HSD)//1    WARING NEED CHECK
    msWriteByteMask( REG_1E28, 0x10, 0x3F );    // od clk
    msWriteByteMask( REG_1E29, 0x14, 0x3F );    // od hsd clk
    #else
    msWriteByteMask( REG_1E28, 0x10, 0x3F );    // od clk
    msWriteByteMask( REG_1E29, 0x10, 0x3F );    // od hsd clk
    #endif
...
void msTMDSInit(void)
{
    ...
    // DVI enable
    #if (DVI_PORT && DVI_PORT==TMDS_PORT_C) || ((HDMI_PORT&TMDS_PORT_C))//(HDMI_PORT && (HDMI_PORT==TMDS_PORT_C))
  ...
  #if 1 // ENABLE_AUDIO_AUTO_MUTE // auto mute setting //coding
  ...
  #endif
  #if 1 //ENABLE_AUDIO_AUTO_FADING // auto fading setting //coding
...
void drvmStar_Init( void )
{
    ...
    msWriteByteMask( SC0_48, 0, BIT4 );                     //Turn off New mode, (Show line tune fast)
  	msWriteByteMask( SC0_6C, BIT7, BIT7|BIT6|BIT5 );	//120911 Modify
...
#define DELAY_LINES  1 //  default: 1
    ...
    if( CURRENT_SOURCE_IS_INTERLACE_MODE() )
    {
        ...
        if (!EXPANSION_MODE_IS_FULL() && !INPUT_IS_WILD_TIMING())//(UserprefExpansionMode!=Expansion_Full)//(USER_PREF_WIDE_MODE!=WM_WIDE) //Jison 110516	//120525 Modify
...
void mStar_SetPanelSSC( BYTE freqMod, BYTE range )
{
    ...
    DRVMSTAR_printData("dwFactor  REG_3852 %x ",msReadByte( REG_3852 ));
    DRVMSTAR_printData("dwFactor  REG_3851 %x ",msReadByte( REG_3851 ));
    DRVMSTAR_printData("dwFactor  REG_3850 %x ",msReadByte( REG_3850 ));
...
#if 0//ENABLE_DUAL_LINK
	if(GetVfreq()>140)
    msWriteBit(REG_381B, _DISABLE, BIT3);
	else
#endif
#if DISABLE_SSC
    msWriteBit(REG_381B, _DISABLE, BIT3); 
...
#if ENABLE_DEBUG
printData("inport00      %d  ", inport);
#endif
		
		if (inport==Input_HDMI||inport==Input_HDMI2)
		{
			msWriteByteMask( REG_PM_B3, BIT2, BIT2 ); // power down DVI CKIN. (port C) (reg_b3[2])
        		msWriteByteMask( REG_PM_B2, 0, BIT2 ); // power on DVI CKIN. (port 0 and port 1) (reg_b2[2])
		}
...
{
	if(inport==Input_HDMI)		//TMDS_PORT_A
	{		
        	msWrite2ByteMask( REG_29C0, 0x1000, 0xFFFF ); // enable DVI0 PLL power, [12:11]: portB/A Data R-term
        	msWriteByteMask( REG_PM_B3, BIT4, BIT5 | BIT4 ); //DVI clock mux: [5]: port 1(portB), [4]: port 0(portA)
        	msWriteByteMask( REG_29C2, BIT0, 0x0F); // Dual link primary/secondary:[3]: 0: port C clock from itself [2]: 0: port A/B clock is from itself, [1:0] port A/B data mux: 10: port B; 01: port A
	...
	else if(inport==Input_HDMI2)		//TMDS_PORT_B
	{
		msWrite2ByteMask( REG_29C0, 0x0800, 0xFFFF ); // enable DVI0 PLL power, [12:11]: portB/A Data R-term
		msWriteByteMask( REG_PM_B3, BIT5, BIT5 | BIT4 ); //DVI clock mux: [5]: port 1(portB), [4]: port 0(portA)
		msWriteByteMask( REG_29C2, BIT1, 0x0F); // Dual link primary/secondary:[3]: 0: port C clock from itself [2]: 0: port A/B clock is from itself, [1:0] port A/B data mux: 10: port B; 01: port A
	...
	else							//TMDS_PORT_C
	{
	       msWrite2ByteMask( REG_29D2, 0x0000, 0x8FF7 ); // enable DVI1 PLL power, [11]: port C Data R-term
        	msWriteByteMask( REG_PM_B3, BIT4, BIT5 | BIT4 ); //DVI clock mux: [5]: port 1(portB), [4]: port 0(portA)
        	msWriteByteMask( REG_29C2, BIT2, 0x07); //2jemery try mod 180930
		//msWriteByteMask( REG_29C2, 0, BIT3); // Dual link primary/secondary:[3]: 0: port C clock from itself [2]: 1: port A/B clock is from port C, [1:0] port A/B data mux: 10: port B; 01: port A
		msWriteByteMask( REG_29D4, BIT0, BIT0); // [0]: enable DVI port 2
        	msWrite2ByteMask( REG_29C0, 0x9FF7, 0x9FF7 ); // enable DVI0 PLL power, [12:11]: portB/A Data R-term
       	msWriteByteMask( REG_2800, BIT6, BIT7|BIT6 ); // [7]: ABSWAP, [6]: ignore_dual_en //????
...
else
{
	...
	msWriteByteMask( REG_2800, BIT7, BIT7|BIT6 ); // [7]: ABSWAP, [6]: ignore_dual_en //????
		...
		{
			msWriteByteMask( REG_2801, BIT6|BIT5, BIT6|BIT5 ); // [6]: DVI_linkb_en, [5]: DVI_dual_manual_en
        	msWriteByteMask( REG_2860, 0, BIT6|BIT5|BIT4 );//2 jemery try mod 180930//
        	//msWriteByteMask( REG_2860, BIT5|BIT4, BIT6|BIT5|BIT4 ); // [6]: DVI_dual_b_ov_en, [5]: DVI_dual_c_ov_en, [4]: DVI_dual_ov_en
		}
...
void I_Gen_Tuning( void )
{
    ...
    //TimeOutCounter = 50;
    SetTimOutConter(50);
    while(!( msReadByte( REG_3269 ) & BIT7 ) && bTimeOutCounterFlag);  // wait for MOD HW calibration done
...
void I_Gen_Tuning( void )
{
   ...
   //MST_printMsg( "I_Gen_Tuning_Start" );
    ...
    for( i = 0; i <= SWING_MASK; i++ ) /*? if LVDS swing > 233mV , then REG_306C[7](C_DDA_OUT)=1*/
    {
		...
		if ( i==0 ||  i==SWING_MASK)
		{
			msWriteByteMask(REG_3260, SWING_MASK/2,SWING_MASK); 					/* Set MOD output swing value for clock channel*/
...
void Power_ModCtrl(BYTE ucSwitch)
    {
...
#if PANEL_QUAD_PORT
        msWriteByteMask(REG_3250,(~ucOnOff)&0x1F,0x1F);
        msWriteByteMask(REG_3264,ucOnOff&0x1F,0x1F );
#elif PanelDualPort
        //msWriteByteMask(REG_3250,(~ucOnOff)&(BIT3|BIT2|BIT0),BIT3|BIT2|BIT0);
        //msWriteByteMask(REG_3264,ucOnOff&(BIT4|BIT3|BIT2),(BIT4|BIT3|BIT2));
        ...
        if( I_Gen_CH == CH3_CLK)
        {
            msWriteByteMask(REG_3250,(~ucOnOff)&(BIT1|BIT0),BIT1|BIT0);
            msWriteByteMask(REG_3264,ucOnOff&(BIT4|BIT0),(BIT4|BIT0));
        }
        else if( I_Gen_CH == CH9_CLK)
        {
            msWriteByteMask(REG_3250,(~ucOnOff)&(BIT2|BIT0),BIT2|BIT0);
            msWriteByteMask(REG_3264,ucOnOff&(BIT4|BIT1),(BIT4|BIT1));
        }
        else if( I_Gen_CH == CH15_CLK)
        {
            msWriteByteMask(REG_3250,(~ucOnOff)&(BIT3|BIT0),BIT3|BIT0);
            msWriteByteMask(REG_3264,ucOnOff&(BIT4|BIT2),(BIT4|BIT2));
        }
        else
        {
            msWriteByteMask(REG_3250,(~ucOnOff)&(BIT4|BIT0),BIT4|BIT0);
            msWriteByteMask(REG_3264,ucOnOff&(BIT4|BIT3),(BIT4|BIT3));
        }
#endif
...
BYTE GetVfreq( void )
{
    ...
    if( g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL || g_SetupPathInfo.ucSCFmtOut == SC_FMT_OUT_3D_PSV || g_SetupPathInfo.ucSCFmtOut == SC_FMT_OUT_3D_PSV_4M )
    {
        DRVMSTAR_printData("V_FREQ_IN:%d", V_FREQ_IN);

        #if (PanelType == PanelLTM270DL06||PanelType == Panel32W_BOEHV320QHM_C80||PanelType == PanelAUOM270DTN01_1||PanelType == PanelLM250WQ1_S2A2 ||PanelType ==PanelAUOM320DVN01_0)//petit  20130626
                   return 60; // Normal In Freq
        #else
...
#if Enable_Expansion && !ENABLE_OVER_SCAN
WORD GetImageOutH()
{
#if 0
	WORD xdata u8HRatio;
    ...
    if (EXPANSION_MODE_IS_FULL() || INPUT_IS_WILD_TIMING() )//(UserprefExpansionMode==Expansion_Full)	//120525 Modify
        return PANEL_WIDTH;
    ...
    #endif
    if (!EXPANSION_MODE_IS_FULL() && !INPUT_IS_WILD_TIMING() )//(UserprefExpansionMode==Expansion_Aspect)	//120525 Modify
    {
    #if 0
		u8HRatio=StandardModeWidth;
		u8VRatio=StandardModeHeight;
	#else
        u8HRatio=4;
        u8VRatio=3;
	#endif
...
void mStar_LPLL1_Setting( const DWORD kHzODCLK )
{
    ...
    XDATA DWORD     u32i_gain;
    XDATA DWORD     u32p_gain;

    //MST_printData( " LPLL1 kHzODCLK = %d", kHzODCLK);
    DRVMSTAR_printData( " kHzODCLK = %d", kHzODCLK);
...
#if FRAME_BFF_SEL==FRAME_BUFFER
        u32i_gain = ( float )( 30*524288/8/(PanelHTotal/PANEL_H_DIV)/GetVtotal()*u8loop_gain*2 )/(float)(( ((msReadByte(REG_3819)>>4) + ((msReadByte(REG_381C)&BIT1)<<3)) +1)*7);
#endif
    }
    #else
    {
...
#if FRAME_BFF_SEL==FRAME_BUFFER
        u32i_gain = ( float )( 30*524288/8/(PanelHTotal/PANEL_H_DIV)/GetVtotal()*u8loop_gain )/(float)(( ((msReadByte(REG_3819)>>4) + ((msReadByte(REG_381C)&BIT1)<<3)) +1)*7);
#endif
    }
    #endif

    u32i_gain = 5 + (long)log(u32i_gain)/log(2);
   ...
   DRVMSTAR_printData( " u32factor H= %x", (WORD)(u32factor>>16));
   DRVMSTAR_printData( " u32factor L= %x", (WORD)(u32factor));
#if (ENABLE_RTE&& (OD_MODE == OD_MODE_333)) && PanelDualPort
    msWriteByteMask(REG_3883, _BIT2, _BIT2);
    //msWriteByteMask(REG_3882, 0, _BIT2);
#endif
...
void mStar_SetTimingGen( WORD u16HDE_OP21, WORD u16VDE_OP21, BOOL OP21_FreeRun)
{
    ...
    DRVMSTAR_printData( " TimingGen ucSCPathMode === %d", g_SetupPathInfo.ucSCPathMode);
    ...
    if ( ((g_SetupPathInfo.ucSCPathMode == SC_PATH_MODE_1) && (!OP21_FreeRun)) || (!g_SetupPathInfo.bFBMode)) //case 12, FBL
    {
        ...
        if( OP21_FreeRun ) // frame buffless mode, free run
        {
            ...
            msWrite2ByteMask(SC0_1E, 0xFFF, SC_MASK_V); // output vtotal
...
#if Enable_Expansion && (FRAME_BFF_SEL==FRAME_BFFLESS) )||Enable_Rotate180Func//JisonWideMode
        if (!EXPANSION_MODE_IS_FULL() && (!OP21_FreeRun) && !INPUT_IS_WILD_TIMING())//(UserprefExpansionMode!=DefExpansion&& (!OP21_FreeRun))//(USER_PREF_WIDE_MODE!=WM_DEFAULT && (!OP21_FreeRun))	//120525 Modify
        {
    ...
    else // Case 3456, FB free run
    {
...
#if 0//ENABLE_NV_DVI_3D
        msWriteWordMask(SC0_21, ( PanelHeight  + 1 ), 0x7FF); // vsync start
        msWriteWordMask(SC0_23, ( PanelHeight  + 3 ), 0xFFF); // vsync end
#else
        ...
        if(OverScanSetting.Enable && (g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL))   //2011026 Xm, ASUS VG278H_1Chip,  C014, C543
        {
        ...
        msWrite2ByteMask(SC0_16, ( ( PanelHStart/PANEL_H_DIV ) + (PANEL_WIDTH/PANEL_H_DIV) - 1 ), 0xFFF); //panel Hor. DE end

#if Enable_Expansion && (FRAME_BFF_SEL==FRAME_BUFFER) //JisonWideMode
        if (!EXPANSION_MODE_IS_FULL() && (!OP21_FreeRun) && !INPUT_IS_WILD_TIMING())//(UserprefExpansionMode!=Expansion_Full && (!OP21_FreeRun))	//120525 Modify
        {
            WORD u16ImageOutH=GetImageOutH();
            WORD u16ImageOutV=GetImageOutV();
            // for 1:1 or 16:9
            WORD u16Offset = (PANEL_HEIGHT - u16ImageOutV);
    #if DOWN_COUNTER_V_SYNC
            msWriteByteMask(SC0_1F, 0, BIT7);
            msWrite2ByteMask(SC0_20, ( uwV_total_OP22 - (u16Offset/2) - 1 - 3 ), 0x7FF); // vsync start
            msWrite2ByteMask(SC0_22, ( uwV_total_OP22 - (u16Offset/2) - 1 - 2 ), 0xFFF); // vsync end
    #else //not verify yet
            msWrite2ByteMask(SC0_20, ( uwV_total_OP22 - (u16Offset/2) - PanelVStart ), 0x7FF); // vsync start
            msWrite2ByteMask(SC0_22, ( uwV_total_OP22 - (u16Offset/2) - PanelVSyncBackPorch ), 0xFFF); //vsync end
    #endif
			if(EXPANSION_MODE_IS_ASPECT())// for 4:3 only		//130808 Henry Follow Demo Code
				msWrite2Byte(SC0_10, 0x00); //panel V. start
			else
           		msWrite2Byte(SC0_10, (uwV_total_OP22 - (u16Offset/2) - 1 ));
            msWrite2Byte(SC0_14, (PANEL_HEIGHT - (u16Offset/2) - 1));
            msWrite2Byte(SC0_1A, (PANEL_HEIGHT - u16Offset - 1));

            msWrite2ByteMask(SC0_18, ( ( PanelHStart + ( PANEL_WIDTH-u16ImageOutH )/2 )/PANEL_H_DIV ), 0xFFF);
            msWrite2ByteMask(SC0_1C, ( ( ( PanelHStart + ( PANEL_WIDTH-u16ImageOutH )/2 + u16ImageOutH )/PANEL_H_DIV ) - 1 ), 0xFFF);
        }
        else
#endif
        {
    ...
    #endif
            msWrite2Byte(SC0_10, 0x00); //panel V. start
            msWrite2ByteMask(SC0_14, ( PANEL_HEIGHT - 1 ), 0x7FF); // panel Vert. DE end

            msWrite2ByteMask(SC0_1A, ( PANEL_HEIGHT - 1 ), 0x7FF); //image V. end
            msWrite2ByteMask(SC0_18, ( PanelHStart/PANEL_H_DIV), 0xFFF); //image H. start
            msWrite2ByteMask(SC0_1C, ( ( PanelHStart/PANEL_H_DIV ) + (PANEL_WIDTH/PANEL_H_DIV) - 1 ), 0xFFF); //image H. end
        }
#endif

    }
#if ENABLE_120Hz_OD //For 144Mhz	
	{
		WORD vfreq, hfreq;
		...
		if((SrcModeIndex == MODE_1920x1080P_144Hz ||(vfreq>1300 && vfreq<1500))&&  (g_SetupPathInfo.wImgSizeOutH==PanelWidth &&  g_SetupPathInfo.wImgSizeOutV==PanelHeight))
		{
			//DownCounterVsync
			msWriteByteMask(SC0_1F, BIT7, BIT7);
			msWrite2Byte(SC0_10, 0x00);		   
...
void mStar_SetIVS( void )
{
    ...
    else if( (g_SetupPathInfo.ucSCFmtOut == SC_FMT_OUT_3D_PSV_4M)
     && (VFreqIn == VFREQ_4MFRC) )
    {
        u16IVS = (g_SetupPathInfo.wImgSizeInV+u16Sample_vst)-
            (WORD)((((DWORD)u16Vtotal_in * g_SetupPathInfo.wImgSizeOutV) /PanelVTotal*VFreqIn)/GetVfreq())+10;
    }
    else
    {
...
void mStar_CheckFastClock(WORD u16hPeriod)
{
    ...
    if ( (g_SetupPathInfo.ucSCPathMode == SC_PATH_MODE_1) || (!g_SetupPathInfo.bFBMode) )
    {
        ...
        if ( g_SetupPathInfo.bFBMode
            ...
            && !(!EXPANSION_MODE_IS_FULL() && (g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL && !INPUT_IS_WILD_TIMING()))//(UserprefExpansionMode!=Expansion_Full && (g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL))	//120525 Modify
            #endif
            )
        {
            if( IS_FMT_IN_SBS())
...
void drvmStar_SetupFreeRunMode( void )
{
    ...
    #if Enable_Rotate180Func	//3 20140305  wf8421 解决freerun模式下没倒屏的问题
    msWriteByteMask(SC0_9F, 1, _BIT0); // Using OP2-1
    #else
...
/*  
      ...
      mStar_LPLL1_Setting( ((DWORD)PanelVsyncFreq * PanelVTotal * PanelHTotal + 500)/1000 ); // output dot clock in KHz
  
#elif (!PanelDualPort && !PANEL_QUAD_PORT) || (PanelTTL)
      ...
      msWriteByteMask(REG_1E36, 0x24, 0x3F);
      msWriteByteMask(REG_1E37, 0x2C, 0x3F);
#if (!PanelDualPort) || (PanelTTL) // Single Port
      mStar_LPLL1_Setting( ((DWORD)PanelVsyncFreq * PanelVTotal* PanelHTotal + 500)/1000 ); // output dot clock in KHz
#else
      mStar_LPLL1_Setting(  ((DWORD)PanelVsyncFreq * PanelVTotal* PanelHTotal + 500)/1000/PANEL_H_DIV );
#endif
#endif   
#else
      //mStar_SetFrameDivider(60); //Jison 110426 for mStar_LPLL1_Setting() read back???
      mStar_LPLL1_Setting( ((DWORD)PanelVsyncFreq * PanelVTotal* PanelHTotal + 500)/1000/PANEL_H_DIV ); // output dot clock in KHz
#endif    */
#if OD_MODE_333_HSD
#if (ENABLE_RTE&& (OD_MODE == OD_MODE_333))
      msWriteByteMask(REG_1E36, 0x20, 0x3F);
  ...
  //  mStar_LPLL1_Setting(  ((DWORD)PanelVsyncFreq * PanelVTotal* PanelHTotal + 500)/1000/PANEL_H_DIV );
    ...
    //g_SetupPathInfo.bFBMode = 1;    //20111003 Xm, ASUS VG278H_1Chip,
...
void drvmStar_SetupInputPort_DVI(void)
{
    ...
    msWriteByte( SC0_04, BIT3  | BIT7 |BIT4);		//2  Jemery 181123 mod
...
void drvmStar_SetupInputPort_HDMI(void)
{
    ...
    msWriteByteMask( REG_2E02, IP_MUX_DVI_A << 4, 0xF0 );    // IPmux 2
    msWriteByteMask( REG_1E3E, (CLK_DVI_BUFF_DIV2<<6)|(CLK_DVI_BUFF_DIV2<<4), 0xF0 );
    msWriteByte(SC0_02, NIS_B|ISEL_HDMI_444|SCALER_STYPE_HV);//msWriteByte(SC0_02, NIS_B|ISEL_DVI|SCALER_STYPE_HV);//120830 Modify
    //msWriteByte(SC0_02, NIS_B|ISEL_HDMI_444);
