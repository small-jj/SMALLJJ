...
#define WaitOutputFpllStable()  {FPLLTimeoutCounter=FPLL_TIMEOUT;while(!(msRead2Byte(REG_3854)==0x1F4) && (FPLLTimeoutCounter));}//if(!DDCCI_MCCS_CMD_FLAG) {g_u16DelayCounter=5000;while(!(msRead2Byte(REG_3854)==0x1F4) && (g_u16DelayCounter));}
