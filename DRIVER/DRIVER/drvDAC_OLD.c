...
void msAudioDAC_Init( void )
{
    ...
    */
    ...
    #if SWAP_LR_CHANNL
    msWriteByte(REG_2C05, 0x10);   // enable PCM gen(1KHz)
    #else
    msWriteByte(REG_2C05, 0x00);   // enable PCM gen(1KHz)
    #endif
    //[2]: fading gain of DPGA; [0]: DPGA enable
    // 120425 coding modified for dac output too slow
   ...
   uVolume=volume;
    ...
    switch( chsel )
    {
        case E_AUDIO_LEFT_CH:
            msWriteByte( REG_2C60, ( uVolume & 0x3F ) );
            msWriteByte( REG_2C69, 0x0F );
            msWriteByte( REG_2C69, 0x0A );
            break;

        case E_AUDIO_RIGHT_CH :
            msWriteByte( REG_2C61, ( uVolume & 0x3F ) );
            msWriteByte( REG_2C69, 0x0F );
            msWriteByte( REG_2C69, 0x0A );
            break;
...
*/
	...
	#if ENABLE_DP_INPUT		//121121 Modify
	if(CURRENT_INPUT_IS_DISPLAYPORT())
		msWriteByteMask( REG_05CD, BIT4, BIT4 );// Set DP Audio Source
	else
		msWriteByteMask( REG_05CD, 0, BIT4 );	// Set HDMI Audio Source
	#endif
}
...
if(val)
{
    msWrite2Byte(REG_2C2A, 0x00); // L-CH: -448
    msWrite2Byte(REG_2C2C, 0x00); // R-CH: -448
}
