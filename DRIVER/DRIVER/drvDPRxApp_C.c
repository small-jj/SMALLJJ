...
bit bDPRxStableTimeoutFlag = 0; 	//130912 nick
WORD _DPDATATYPE_ u16FastTrainingCntDown=0;
#endif

DWORD _DPDATATYPE_ DPTP1Cnt = 0;
DWORD _DPDATATYPE_ DPTP2Cnt = 0;
WORD _DPDATATYPE_ u16CDR_HPDCnt = 0; 	//130912 nick
WORD _DPDATATYPE_ u16Skew_HPDCnt = 0; // Add by CCJ 	//130912 nick
WORD _DPDATATYPE_ u16Audio_BWCnt = 0; // Add by CCJ 	//130912 nick
WORD _DPDATATYPE_ u16HDCP_Cnt = 0; 	//130912 nick
//130912 nick
...
code RegTbl2Type tComboDPMuxTbl[]=
{
  ...
  //{REG_17AE, 0x4080, 0xFFFF},
   ...
   {REG_17A4, 0x3100, 0xFFFF},
    ...
    {REG_01CE, 0x0000, 0xFFF0},     // Clock R-term couldn't be set in the initial table 	//130912 nick

    {REG_06C0, 0x1F08, 0xFFFF},     // SQ debounce.
    {REG_067C, 0x0008, 0x00FF},     // DPCD Program Disable
    {REG_01BB, 0x0030, 0x0030},    //enable DP differetial Rterm   //20171109
 //====================================================
...
#if  EDID_Panel_238
BYTE code  EDID[256] =       //Port B  EDID	//130611 nick
{
0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,
0x04,0x21,0x00,0x00,0x01,0x00,0x00,0x00,
0x29,0x1B,0x01,0x04,0xA5,0x35,0x1D,0x78,
0x3B,0xBA,0x25,0xA3,0x56,0x50,0xA0,0x28,
...
,

};
BYTE code  EDID1[256] =         //Port C  EDID	//130611 nick
{
0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,
0x04,0x21,0x00,0x00,0x01,0x00,0x00,0x00,
0x29,0x1B,0x01,0x04,0xA5,0x35,0x1D,0x78,
...
0x01,0x1B,0x01,0x04,0xA5,0x36,0x1E,0x78,
0x3B,0xBA,0x25,0xA3,0x56,0x50,0xA0,0x28,
0x0E,0x50,0x54,0xBF,0xEF,0x00,0xD1,0xC0,
0xB3,0x00,0x95,0x00,0x81,0x80,0x81,0x40,
0x81,0xC0,0x01,0x01,0x01,0x01,0x02,0x3A,
...
0x20,0x20,0x20,0x20,0x00,0x00,0x00,0xFF,
0x00,0x41,0x55,0x39,0x30,0x37,0x30,0x31,
0x5A,0x30,0x30,0x30,0x30,0x31,0x01,0x14,
0x02,0x03,0x18,0xF1,0x4B,0x01,0x03,0x05,
0x14,0x04,0x13,0x1F,0x12,0x02,0x11,0x90,
0x23,0x09,0x07,0x07,0x83,0x01,0x00,0x00,
0x02,0x3A,0x80,0x18,0x71,0x38,0x2D,0x40,
...
void DPRxInit( void )
{
    ...
    if(g_bDoDPInit)
   {
    ...
    for(Aux=0;Aux<=0x100;Aux+=0x100)
     {
             ...
             msWriteByteMask( REG_0603+Aux, BIT3,BIT3|BIT1  );  	//130912 nick                     // DP Number of Receiver Ports // TP3 not support.
             ...
             msWriteByteMask( REG_06AA+Aux, BIT7, BIT7);                         // AUX command mode turn on
         ...
         if((DPNUM >=0x08)&&(DPLEG==0x61))
          {
            ...
            msWriteByteMask( REG_01BB, BIT5,BIT5);                  //Common mode Voltage for Port 2
             ...
             //msWriteByteMask( REG_06A9+Aux, BIT0|BIT2, BIT0|BIT1|BIT2|BIT3);       // R-term  20171109
	      ...
	      // msWriteByteMask( REG_06AB+Aux, 0x05, 0x07);     	 //20171109      Aux tx swing    //130912 nick           // Set AUX TX swing
		msWriteByteMask( REG_0642+Aux, BIT0, BIT0);     //130912 nick                       // Enable GUID Read/Write
    		#if  DP_GUID_SW
    ...
    msWriteByte( REG_1321, 0x08 );		 	//130912 nick																										  //Htotal Learning Cnt
...
void DPRxInit_PM( void )
{
	...
	// 130910 nick
    msWriteByte( REG_175C, 0x3C );// msWriteByte( REG_175C, 0xFF );  // mask INT
    msWriteByte( REG_175D, 0xF8 );//msWriteByte( REG_175D, 0xFF );  // mask INT
//msWriteByte( REG_175C, 0xFF );  // mask INT
// msWriteByte( REG_175D, 0xFF );  // mask INT
...
void DPRxHDCPLoadKey( void )
{
    ...
    #if DP_USE_INTERNAL_HDCP ||ENABLE_DP_HDCP_IN_FLASH
    BYTE xdata ucValue;
    #endif
	...
	#if ENABLE_DP_HDCP_IN_FLASH	//121115 Modify
	{
		for( i = 0; i < 5; i++ )
		{
			ucValue=FlashReadByte( HDCPKEYSET_START +i);//NVRam_ReadByte(RM_HDCP_KEY_BASEADDR1+i,	&value);
			msWriteByte( REG_150E, ucValue);
			msWriteByteMask( REG_153A,BIT3, BIT3 );   
			msWriteByte( REG_06A0 + i, ucValue);           //Off-Line Key
			msWriteByte( REG_07A0 + i, ucValue );           //Off-LineKey    
		}

		for(; i < 289; i++ )
		{
		  #if ENABLE_USE_ISPTOOL_Write_HDCP
	        ucValue = FlashReadHDCPByte( IspWriteHDCPkey_Addre + i );	//120105 Modify
	        #else
			ucValue=FlashReadByte( HDCPKEYSET_START +i);//NVRam_ReadByte(RM_HDCP_KEY_BASEADDR1+i,	&value);
	        #endif
			msWriteByte( REG_150E, ucValue);
...
void DPRxHandle( void )
{
    ...
    msWriteByte( REG_060B, m_ucState);
    ...
    switch( m_ucState )
    {
        ...
        case eSTATE_MEASURE:
            ...
            if(1)
            {
				...
				DPMSA_Reset();
				msWriteByte( REG_170B, 0x0A );                  //set BW  	//130912 nick
				SetAudio_BWCnt(2000); 									//130912 nick
                   ...
                   if (DP_GetSquelch(1) )
                    {
                        if (bDPRxStableTimeoutFlag==0 && !gDPInfo.bSquelchStable)
                            {
                                ...
                                {
                                    SetDPRxStableTimeout(Times);  	//130912 nick                           
                    ...
                    else
                        {
                            gDPInfo.bSquelchWaitStable=0;
                            gDPInfo.bSquelchStable=0;
                            SetDPRxStableTimeout(0);        	//130912 nick                     
...
void DPRxHdcpReAuthen(void)
{
      ...
      if(((msReadByte(REG_1882)&0x03)==0) && DPRxCheckLock()&&(msReadByte(REG_18C0)&BIT2))
      {
                if(g_bDPHDCPIRQ ==0)
               {
                    SetHDCP_Cnt(1000); 	//130912 nick
                    //u16HDCP_Cnt = 1000;
                    //g_bDPHDCPIRQ = 1;
               }

               if(g_bDPHDCPIRQ && (u16HDCP_Cnt==0)) 	//130912 nick
            {
                    DPRxHPDDisable();
                    Delay1ms( 5);
                    DPRxHPDEnable();
     ...
     BYTE xdata ucTemp=1;
		...
		for( i = 0; i < 256; i++ )
		{
          ...
          //============================================
			...
			for( i = 0; i < 256; i++ )
			{
				#if ENABLE_DP_EDID_IN_FLASH
				#if ENABLE_DP_EDID_IN_FLASH_3B	//131101 Modify
				Flash_Read_Factory_KeySet( FLASH_KEY_DDCDP, TRUE, i, 1, &ucValue, 1 );		  
				#else
				ucValue=FlashReadByte(DP_EDID_START+i);
				#endif
				if(i >= 128)
				{ 
					msWriteByteMask( REG_3EEB, 0x20, 0xF0);
...
void DPRxHPDEnable( void )
{
     ...
     {     
           SetPortCHPDHigh();                                                                           // HPD ouput enable and set Low  GPIO31      
     }
    else 
     {
...
{
    return msRead2Byte(REG_14B8) & 0x7FFF;//20180111
}
   ...
   if( g_bDPLoseCDRLockIRQ ) // Lock fail have occured
    {
        if( !DPRxCheckLock() )
        {
             if(( u16CDR_HPDCnt == 0 ) && ( (msReadByte( REG_1882 )&0x03) == 0 ) ) 	//130912 nick
            {
    ...
    if( g_bDPInterlaneSkewIRQ ) // Lock fail have occured
    {
        if( DPRxCheckLock() )
        {
            if(( u16Skew_HPDCnt == 0 ) && (( msReadByte( REG_1882 )&0x03) == 0 ) ) 	//130912 nick
            {
...
void DPRxAudioHandle( void )
{
    ...
    if( g_bDPLANELOCK == TRUE )
    {
         ...
         //################  Audio M/N example ###################
	      ...
	      if(u16Audio_BWCnt ==0) 	//130912 nick
	      {
                      ...
                      if( g_bDPAUPLLBIGChange && ( !gDPInfo.bDPNoAudioStrm ) )
                        {
                              ...
                              if((DPAUM_Temp == DPAUM)&&(DPAUN_Temp == DPAUN))
                             {
                                ...
                                DP_printData( "\r\nDP_Audio_bigchange%x ", 0 );
                                    ...
                                    DPKM = GetPow2((msReadByte( REG_1701 )  & 0x0F )); //KM 	//130912 nick

                                    DPFREQ = (64)  * ( 214.77 / LinkRate ) * ( DPKM * DPAUN ) / ( DPKP * DPAUM ) * DPFBDIV; 	//130912 nick
                                  ...
                                  //  msWriteByte( REG_170B, 0x5A );                  //set BW  	//130912 nick

                                    msWriteByte( REG_1707, 0x20 );           //update Freq
                                    msWriteByte( REG_1707, 0x00 );           //update Freq
                                            gDPInfo.bAupllRun=1;
                                    
                                    msWriteByte( REG_170B, 0x0A ); 	//130912 nick
                                    SetAudio_BWCnt(500); 			//130912 nick
...
void DPISR( void )
{
                ...
                if( msRegs[REG_1756] & BIT7 )         // Training P1
                {
                 ...
                 //      msRegs[REG_17A8] = msRegs[REG_17A8] & (~(BIT0|BIT2));      //Override Ki
                    ...
                    while((( msRegs[REG_1882] & 0x03 ) == 0x01 ) && ( DPTP1Cnt < 50000 ) ) // while pattern 1 is On-Going
                    {
                        if(DPTP1Cnt > 49990)
            ...
            if( msRegs[REG_1757] & BIT1 ) // InterlaneSkew
            {
                ...
                SetSkew_HPDCnt(1500);	 	//130912 nick	
            ...
            if( msRegs[REG_1757] & BIT2 ) // CDRLoseLock
            {
                ...
                SetCDR_HPDCnt(1500); 	//130912 nick
                //u16CDR_HPDCnt = 1500;
                //bCDR_HPDCntFlag=1;
        ...
        else
        {
            ...
            // ##############################################################
                ...
                if( msRegs[REG_0690 + Aux] & BIT0 )
                {
                    ...
                    msRegs[REG_0692 + Aux ] = BIT0;    //Clear
    ...
    BYTE xdata Count=20;
       ...
       for(i=0;i<sizeof(tComboDPMuxTbl)/sizeof(RegTbl2Type);i++)
        ...
        if(((DPNUM >=0x04)&&(DPLEG==0x61))||((DPNUM >=0x02)&&(DPLEG==0x69)))
          ...
          {
                 msWrite2ByteMask(REG_1751,0x0005,0x0005);
          }
        ...
        //msWriteByteMask(SC0_E6, _BIT6, _BIT6); 	//130912 nick                                                                         // DE Follow mode
       ...
       msWriteByteMask( REG_17AB, BIT4|BIT3, BIT1|BIT4|BIT3 );
        ...
        //=======================================================
           if(SrcInputType == Input_Displayport)                                                                                       //(PortB)
         {
               ...
               DP_printMsg("==>Input_Displayport111111");
         ...
         else  if(SrcInputType == Input_Displayport3)                                                                              // (PortC)
         {
                ...
                #endif
                 DP_printMsg("==>Input_Displayport3333");
         }
