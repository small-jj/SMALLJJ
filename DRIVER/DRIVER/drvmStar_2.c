...
code RegUnitType tblInit[]=
{
    ...
    {REG_3680, (PanelSwapMLSB&BIT6)|(PANEL_SWAP_PN&BIT5)|(BIT2&LVDS_TIMode)|BIT3},
    #else
    {REG_3680, (PanelSwapMLSB&BIT6)|(BIT5)|(BIT2&LVDS_TIMode)|BIT3},
    #endif
    {REG_368A, 0x3F},
...
#if ENABLE_MHL	//130716 xiandi follow demo code
	msWriteByteMask(REG_0200, 0, BIT6); // output low
	msWriteByteMask(REG_0201, 0, BIT6); // output enable
	ForceDelay1ms(2);
	msWriteByteMask(REG_0200, BIT6, BIT6); // output high
	msWrite2ByteMask(REG_0200, BIT14 | BIT6, BIT14 | BIT6); // input
	ForceDelay1ms(2);
#endif		 
...
void I_Gen_Tuning( void )
{
    ...
    //TimeOutCounter = 50;
    SetTimOutConter(50);
    while (! ( msReadByte( REG_36FC ) & BIT7 ) && bTimeOutCounterFlag );     
    ...
    else
    {
        ...
        drvmStar_printData("-----REG_36FA=0x%x\n\r", msReadByte(REG_36FA)&SWING_MASK);  // REG_30FA[5:0] is calibration result		
    }
...
void drvmStar_SetupInputPort_DVI(void)
{
    ...
    msWriteByte(SC0_02, NIS_B|ISEL_HDMI_444|SCALER_STYPE_HV);//(SC0_02, NIS_B|ISEL_DVI|SCALER_STYPE_HV);//(SC0_02, NIS_B|ISEL_HDMI_444);//(SC0_02, NIS_B|ISEL_DVI|SCALER_STYPE_HV); // 120530 coding test
}
...
void drvmStar_SetupInputPort_HDMI(void)
{
    ...
    msWriteByte(SC0_02, NIS_B|ISEL_HDMI_444|SCALER_STYPE_HV);//(SC0_02, NIS_B|ISEL_DVI|SCALER_STYPE_HV);//(SC0_02, NIS_B|ISEL_HDMI_444);
...
{
    BYTE xdata ucCLKINVal, ucDMVal, ucDRtermVal, ucDVIMuxVal, ucADCPD1, ucADCPD2;
    Bool bFlag = TRUE;
