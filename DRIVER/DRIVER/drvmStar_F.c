...
void TMDSISR(void)
{
    ...
    if(_bit5_(msRegs[REG_1757])) // CLK back to stable IRQ
    {
...
#if ENABLE_XTAL_LESS
        if(temp2 > 0x6DF) //1    Jemery 180928 follow demo code  if(temp2 > 0x5D5)
        {
        ...
        if(temp2 > 0x12B) //1    Jemery 180928 follow demo code  add
        {
            MEM_MSWRITE_BYTE_MASK(REG_1BDF, 0x00, 0x80); // Disable Auto Band select EQ
        }
        else
        {
            MEM_MSWRITE_BYTE_MASK(REG_1BDF, 0x80, 0x80); // Auto Band select EQ
            MEM_MSWRITE_BYTE_MASK(REG_1BBA, 0x40, 0xF0); // Auto Band select EQ
            MEM_MSWRITE_BYTE_MASK(REG_1BDF, 0x01, 0x01); // Auto Band select EQ
        }		
#else
        if(temp2 > 0x5C3)//1 Jemery 180928 follow demo code  if(temp2 > 0x4E3)
        {
            MEM_MSWRITE_BYTE_MASK(REG_1F3D, 0x00, 0x80); // Disable Auto Band select EQ
            MEM_MSWRITE_BYTE_MASK(REG_1BB5, 0x01, 0x07); //autoEQ start flag
        }
...
static void msSetupDelayLine(void)
{
    ...
    WORD xdata Hi_Act=GetStandardModeWidth();   //  OK
    WORD xdata Vi_Act=GetStandardModeHeight();   //  OK
    #if 0
