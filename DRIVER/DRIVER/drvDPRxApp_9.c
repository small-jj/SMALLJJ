...
void DPRxAuxInit(void)
{
	   ...
	   msWrite2Byte(REG_1EF0,temp);	
        ...
        msWriteByteMask( REG_0631, BIT1, BIT0|BIT1);                                     // Aux Time out Reply
...
void DPRxMACInit(void)
{
    ...
    msWriteByteMask( REG_1326, BIT7, BIT7);							//FST reset HTT
...
#if 0//ENABLE_DEBUG_TIMING_CHANGE_POINT
  ...
  //  }
      ...
      msWriteByte( REG_3EF8, 0x01);								   // D0 Edid Sram address
       ...
       for( i = 0; i < 256; i++ )
       {
          ...
          Flash_Read_Factory_KeySet( FLASH_KEY_DDCDP, TRUE, i, 1, &ucValue, 1 );
	   #elif ((CHIP_ID==CHIP_TSUM9)||(CHIP_ID==CHIP_TSUMF))&&ENABLE_CABLE_5V_EDID
	      Flash_Read_Factory_KeySet( FLASH_KEY_DDCD, TRUE, i, 1, &ucValue, 1 );
       #else
