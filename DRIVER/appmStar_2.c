...
Bool appmStar_SetPanelTiming( void )
{
    ...
    if(CURRENT_INPUT_IS_VGA())
        sclk = (( DWORD )UserPrefHTotal * MST_HPeriod_UINT ) / (( DWORD )hPeriod * 1000 );//计算获取模拟信号的sclk
    else
    {
        dclk = mSTar_GetInputHTotal();
        sclk = (( DWORD )dclk * MST_HPeriod_UINT ) / (( DWORD )hPeriod * 1000 );//计算获取数字信号的sclk
    }

    PANEL_TIMING_printData( "  sclk=%d", sclk );
    
    height = SC0_READ_IMAGE_HEIGHT();//msRead2Byte(SC0_09)&SC_MASK_V;       读取图像的height
#if ENABLE_CHECK_PANEL_VTOTAL	//Calc uput Vtt need before Interlace mode u16Hight/2
		// calc output vtt
		u16OutVtt = ( ( DWORD ) SrcVTotal * PanelHeight + ( height - 1 ) ) / height;//  计算outputvtotal
#endif
