...
static void msSetupPathParam()
{
    ...
    if( !EXPANSION_MODE_IS_FULL() && (g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL) && !INPUT_IS_WILD_TIMING())//( !EXPANSION_MODE_IS_FULL() && (g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL) )//( (UserprefExpansionMode!=Expansion_Full) && (g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL) )	//130408 Modify
    {
...
static void msSetupPathIP()
{
    ...
    //DB_WBMask(SC0_90, (ucDlyLine)<<4, _BIT6|_BIT5|_BIT4); //delay line //130604 Modify
	#if ENABLE_DP_INPUT
...
void mStar_SetupPathInit()
{
    ...
    g_SetupPathInfo.bMRW422 = 0;//130604 Modify
    g_SetupPathInfo.bMRWR2Y = 0;//130604 Modify
