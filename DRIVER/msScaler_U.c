...
void mStar_SetScalingFactor( void )
{
    ...
    {
        factor = (g_SetupPathInfo.wImgSizeInH/2) - HSC_offset; // factor=((input width-1)*2^20)/(output width-1) + 1
...
//setup wImgSizeOutH & wImgSizeOutV ---begin
    ...
    #endif
    {
    #if ENABLE_OVER_SCAN // 121102 coding test
        g_SetupPathInfo.wImgSizeOutH = OverScanSetting.OverScanH;
        g_SetupPathInfo.wImgSizeOutV = OverScanSetting.OverScanV;
    #else
        g_SetupPathInfo.wImgSizeOutH = GetImageOutH();
        g_SetupPathInfo.wImgSizeOutV = GetImageOutV();
    #endif        

  #if 1//ENABLE_OVER_SCAN		//william130802 For Q2770PQU 4:3 fail
  	#if Enable_Rotate180Func//3   	 lixw 2016-03-19 16:57:36  for Rotate180  4:3  fiel 
	  g_SetupPathInfo.bNormalCase34 = 0;
	#else
        g_SetupPathInfo.bNormalCase34 = 1;
       #endif
        MSSCALER_printMsg("SetCase34");    
...
//setup wImgSizeOutH & wImgSizeOutV ---end
    ...
    if( g_SetupPathInfo.wImgSizeInH > g_SetupPathInfo.wImgSizeOutH )
        ...
        if( IS_DP_FREESYNC()||IS_HDMI_FREESYNC())
        {   
           ...
           g_SetupPathInfo.bMRWDual = FALSE;
  ...
  //  if(msGetDVIDualLinkStatus())   // 20130321 add for Dual Link mode need set to Path 1
    //    g_SetupPathInfo.ucSCPathMode = SC_PATH_MODE_1;
...
static void msSetupPathConfig()
{
    switch(g_SetupPathInfo.ucSCPathMode)
    {
		...
		msWriteByteMask( REG_1E37, 0x28, 0x3F );
            #elif (!PanelDualPort && !PANEL_QUAD_PORT) || (PanelTTL) || (ENABLE_RTE && (OD_MODE == OD_MODE_333))
        ...
        case SC_PATH_MODE_1:
            ...
            /* OP2 Select */
            #if 0//Enable_Expansion && (FRAME_BFF_SEL==FRAME_BUFFER && ASPECT_RATIO_CASE34) // for winde mode 4:3 to full garbage
            if (TIMING_SETUP_FINISH_FLAG)
            {
                Delay1ms(30);
        ...
        case SC_PATH_MODE_3:
            ...
            /* Clock */
            msWriteByteMask(REG_1E36, 0x20, 0x3F);
            #if (!PanelDualPort && !PANEL_QUAD_PORT) || (PanelTTL) || (ENABLE_RTE && (OD_MODE == OD_MODE_333))
            msWriteByteMask( REG_1E37, 0x2C, 0x3F );
            #else
//            msWriteByteMask( REG_1E37, 0x20, 0x3F );   //20121105 recheck
            msWriteByteMask( REG_1E37, 0x28, 0x3F );
...
#if 0// (!PanelDualPort && !PANEL_QUAD_PORT) || (PanelTTL) || (ENABLE_RTE && (OD_MODE == OD_MODE_333))
    msWriteByteMask(REG_1E36, 0x20, 0x3F);
...
#if 0// (!PanelDualPort && !PANEL_QUAD_PORT) || (PanelTTL) || (ENABLE_RTE && (OD_MODE == OD_MODE_333)&& OD_MODE_333_HSD)
    msWriteByteMask(REG_1E28, 0x14, 0x3F);
    msWriteByteMask(REG_1E36, 0x24, 0x3F);
    if( g_SetupPathInfo.ucSCPathMode != SC_PATH_MODE_1 )
        msWriteByteMask(REG_1E37, 0x2C, 0x3F);
    msWriteByteMask(REG_1E41, 0x10, 0x3F);
#endif
#if (ENABLE_RTE && (OD_MODE == OD_MODE_333)&&!OD_MODE_333_HSD)
    msWriteByteMask(REG_1E28, 0x10, 0x3F);
    msWriteByteMask(REG_1E29, 0x14, 0x3F);
#endif
...
static void msSetupPathIP()
{
	...
	#if 0//ENABLE_DUAL_LINK//for DP 1680x1050/60Hz flick		// 修改DP 抖动问题
...
#if (ENABLE_FREESYNC&&ENABLE_DP_INPUT)
    if((UserprefFreeSyncMode==FreeSyncMenuItems_On)&&(CURRENT_INPUT_IS_DISPLAYPORT()))
    { BYTE xdata Retry, Tempvalue;
       
        Tempvalue=DPRxGetDRRFlag();
    
       if(!Tempvalue)//mqsh 20170421 for RX480 dp freesync on sometime get DRRFlag error
       {
        ...
        if(Tempvalue)//freesync
        {
            BYTE u8SampleRangeVStartOffset = (msRead2Byte(SC0_7E)) + (4 - ucDlyLine); //SC0_7E
...
static void msSetupPathMRWBaseWR()
{
    ...
    msWrite2Byte(SC6_D2, (MIU_FB_ADDR_END-1)>>12);
	 ...
	 msWrite3Byte(MRW_FLIP_MSB_OFFSET_ACT, MIU_ADDR_OFFSET(MIU_FB_FLIP_ADDR_MSB));
	 msWrite3Byte(MRW_FLIP_LSB_OFFSET_ACT, MIU_ADDR_OFFSET(MIU_FB_FLIP_ADDR_LSB));
#endif
...
static void msSetupPathMRW()
{
    ...
    #if Enable_Rotate180Func	
    msWriteByte(SC6_F4, ucReg|(_BIT3|_BIT2));
    #else
    msWriteByte(SC6_F4, ucReg);
    #endif

    
#if 0//Enable_Expansion && (FRAME_BFF_SEL==FRAME_BUFFER) && !ENABLE_OVER_SCAN			//william130802 For Q2770PQU 4:3 fail
    if( !EXPANSION_MODE_IS_FULL() && (g_SetupPathInfo.ucSCFmtIn==SC_FMT_IN_NORMAL) && !INPUT_IS_WILD_TIMING() )//( (UserprefExpansionMode!=Expansion_Full) && (g_SetupPathInfo.ucSCFmtIn==SC_FMT_IN_NORMAL))	//120525 Modify
    {
        msWrite2Byte(SC6_DC, PANEL_WIDTH);
        msWrite2Byte(SC6_DE, PANEL_HEIGHT);
        msWrite2Byte(SC6_E0, PANEL_WIDTH);
        msWrite2Byte(SC6_E2, PANEL_HEIGHT);

        msWrite2Byte(SC6_AE, PANEL_WIDTH);
        msWrite2Byte(SC6_B0, PANEL_WIDTH);
    }
    else
#endif
    {
...
static void msSetupPathOD()
{
     ...
     && (OD_MODE == OD_MODE_565) )
    {
		...
		if(CURRENT_INPUT_IS_DISPLAYPORT())
			msWrite2Byte(REG_1248, 0xF8FF); // for DP 3D
		else
			msWrite2Byte(REG_1248, 0xFE77); // for nvidia.  0xFE37
	#endif		  
    }
    else
    {
