...
void mStar_SetScalingFactor( void )
{
    ...
    if(!EXPANSION_MODE_IS_FULL() && !INPUT_IS_WILD_TIMING())
    {
...
#if Enable_Expansion
    if( !EXPANSION_MODE_IS_FULL() && (g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL) && !INPUT_IS_WILD_TIMING())
    {
...
static void msSetupPathConfig()
{
   ...
   //1 jemery follow demo code  180928 del
...
#if  1 	//20191008 Update
if(SrcModeIndex == MODE_800x600_56_VESA)
	DB_WB(SCD_68, 0x04);
else if(SrcModeIndex == MODE_640x480_75_VESA)
	DB_WB(SCD_68, 0x03);
else if(SrcModeIndex == MODE_800x600_75_VESA)
	DB_WB(SCD_68, 0x03);
else if(SrcModeIndex == MODE_832x624_75Hz_MAC||SrcModeIndex == MODE_832x624_75_MAC)
	DB_WB(SCD_68, 0x03);
else if(SrcModeIndex == MODE_1024x768_72_VESA||SrcModeIndex == MODE_1024x768_75_VESA||SrcModeIndex == MODE_1024x768_75_MAC)
	DB_WB(SCD_68, 0x03);
else if(SrcModeIndex == MODE_1152x864_75_VESA)
	DB_WB(SCD_68, 0x03);
else if(SrcModeIndex == MODE_1280x960_60Hz_VESA||SrcModeIndex == MODE_1280x960_60Hz_VESA1||SrcModeIndex == MODE_1280x960_70Hz_VESA||MODE_1280x960_75Hz_VESA)
	DB_WB(SCD_68, 0x03);
else if(SrcModeIndex == MODE_1280x1024_60_VESA||SrcModeIndex == MODE_1280x1024_75_VESA)
	DB_WB(SCD_68, 0x03);
else
#endif

   DB_WB(SCD_68, 0x02);	   
   #message "190313 Jemry modify it"
    //DB_WB(SCD_68, 0x08);    //1 jemery 180928 follow demo code  mod
