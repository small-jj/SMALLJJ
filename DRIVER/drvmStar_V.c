...
void drvmStar_Init( void )
{
	...
	#if 0//EnablePanelServiceMenu	//120112 Modify
...
void drvDVI_PowerCtrl(DVI_PowerCtrlType ctrl)
{
    ...
    {
        // DVI power down
        #if 1 //all source in put , VGA DC OFF  , but DVI  CKIN/PLL  not off.
        msWriteByteMask(REG_PM_B2, BIT2|BIT3|BIT0, BIT2|BIT3|BIT0); //20130319 Modify Added BIT0// power down DVI CKIN/PLL band-gap.
	#else
...
void mStar_SetScalingFactor( void )
{
    ...
    if(!EXPANSION_MODE_IS_FULL() && !INPUT_IS_WILD_TIMING())//( UserprefExpansionMode == Expansion_Aspect && ExpansionFlag )
    {
        //Expansionwidth = (( DWORD )Factor_height * width ) / ( 100 );
        	Expansionwidth = (( DWORD )PanelHeight * width * 3) / ( 4 * height );
...
void I_Gen_Tuning( void )
{
    ...
    for (i=0; i<=SWING_MASK; i++)   /*? if LVDS swing > 233mV , then REG_306C[7](C_DDA_OUT)=1*/
    {
		...
		if ( i==0 ||  i==SWING_MASK)
		{
			msWriteByteMask(REG_306C, SWING_MASK/2|BIT6,SWING_MASK|BIT6);						/* Set MOD output swing value for clock channel*/
