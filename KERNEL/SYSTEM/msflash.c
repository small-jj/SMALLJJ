...
///////////////////////////////////////////////////////////////////////////////
//这段代码是一个简单的函数，用于从Flash中读取一个字节的数据。代码中定义了一个指向Flash地址0x0000的指针：xfr_FlashCode。
//FlashReadByte函数接受一个16位的地址参数wAddr，并返回该地址上Flash中存储的字节数据。
//s具体而言，函数通过将地址wAddr作为索引来访问xfr_FlashCode指针所指向的内存位置，并返回该位置上的数据。
...
void Flash_WriteTbl( Bool bDoWP, WORD wAddr, BYTE *buffer, WORD count )
{
    ...
    if( bDoWP )
        FlashEnableWP();// 2005/5/11 �W�� 10:19:43 by keng
...
/*
 *****************************************
 * @Description ：读Flash数据的函数
 * @Parameter1 ：wAddr -> 要读的地址
 * @Parameter2 ：buffer -> 读到哪个buf
 * @Parameter3 ：count -> 读的个数
 *****************************************
 */
...
//////////////////////////////////////////////////////////////////////////////
//根据你所传进来的Type在四个动态block中查找，查找到后返回地址
WORD Flash_SearchBufferAddr( FLASH_BUFFTER_TYPE ucType )
{
    BYTE ucTemp;

    for( ucTemp = 0; ucTemp < FDATA_DYNAMIC_SECTOR_NUM; ucTemp++ )//4
    {
...
//////////////////////////////////////////////////////////////////////////////
//waddr:起始地址
//wstartidx,wendidx:第几组开始到第几组结束，每组有多少个成员看结构体而定
//wrange:一组的大小
//ucident,ucchecksum:从某组第一位开始看是不是0xff,到一组结束最后一位是不是0xff,不是则继续下一组继续以此类推
//一个bank 64k（0000H-FFFFH）,一个block为4k,一个指针访问的区间为一个Bank的大小
//
...
//////////////////////////////////////////////////////////////////////////////
//得到HDCP_EDID/Factory/MonitorSetting/MonitorSetting2/ModeSetting存放在Flash中的Block起始地址.
//其中HDCP_EDID固定Block起始地址为0x7F000，Factory 固定Block起始地址为0x7E000
void Flash_GetBufferAddr(void)
	{
		WORD wAddr;
		FLASH_HDCP_EDIDAddr = FDATA_HDCPKEY_START_ADDR;//0xf000
		FLASH_FactorySettingAddr = FDATA_FACTORY_ADDR;//0xe000
		...
		wAddr = Flash_SearchBufferAddr(FBufType_MonitorSetting);//0x88//地址搜索函数搜索地址
		if(wAddr)
		...
		wAddr = Flash_SearchBufferAddr(FBufType_MonitorSetting2);//0x44
		if(wAddr)
		...
		wAddr = Flash_SearchBufferAddr(FBufType_TimingMode);//0x22
		if(wAddr)
		...
		wAddr = Flash_SearchBufferAddr(FBufType_EmptyBuffer);//0xff
		if(wAddr)
		...
		// 091022 coding, free buffer type should be 0xff or 0x00
			...
			{
				if((wAddr = Flash_SearchBufferAddr(FBufType_FullBuffer)) >= FDATA_DYNAMIC_ADDR)//0x00,0xa000
					FLASH_FreeBufferAddr = wAddr;
			}
...
//////////////////////////////////////////////////////////////////////////////
//检查HDCP_EDID/Factory/MonitorSetting/MonitorSetting2/ModeSetting中是否有未分配到Block，没有则重新分，确保合法
...
//////////////////////////////////////////////////////////////////////////////
//若各个地址对应的Block没有或不等于起始Flag，则对相应的Block进行擦除动作，并写入相应的起始Flag
...
//ture获得“空的”数据的状态位的位置,flase获取有效数据的状态位的位置
//////////////////////////////////////////////////////////////////////////////、
/*
 **********************************************************************************************
 * @Description ：算ucKeyIndex，可以类似理解为偏移量的倍数，有效数据是第几组，要偏移多少个空间size
 * @Parameter1 ：ucType -> key类型
 * @Parameter2 ：bWrite -> 传入false时，空间满了不会返回最后一组数据，直接返回0；
 * 						   空间未满的话返回有效数据的空间偏移倍数
 ***********************************************************************************************
 */
...
//                  Exchange buffer address FLASH_FreeBufferAddr and FLASH_FactorySettingAddr
////////////////////////////////////////////////////////////////////////////////
//	将工厂设置、HDCP 密钥、DDCA、DDCD 密钥写入空闲缓冲区、
// 交换缓冲区地址 FLASH_FreeBufferAddr 和 FLASH_FactorySettingAddr
void Flash_BackupFactorySetToBuffer(BYTE ucBufferType)
{
	...
	FlashDisableWP( FLASH_FreeBufferAddr );
     ...
     {	
        ucKeyIndex = Flash_GetKeyIndex(FLASH_KEY_FACTORY, FALSE);//返回有效数据那一组
     	if(FlashReadByte(FACTORY_STATUS_ADDR+ucKeyIndex) == FLASH_KEY_VALID
     		&& FlashReadByte(FACTORY_BUF_STATUS_ADDR) == FLASH_KEY_EMPTY)
     	{
     		Flash_MoveTbl(FALSE, FACTORY_START+(WORD)ucKeyIndex*FACTORY_SIZE, FACTORY_BUF_START, FACTORY_SIZE);//有效数据放入freebuffer
     		FlashWriteByte(FALSE, FACTORY_BUF_STATUS_ADDR, FLASH_KEY_VALID);//写入数据标识
     ...
     #endif
     	FlashWriteByte( FALSE, FLASH_FreeBufferAddr, FBufType_FactorySetting );//标识空间的类型
     	FlashEnableWP();//使能写引脚
     	FlashWriteByte( TRUE, FLASH_FactorySettingAddr, 0x00 );//旧地址的最后一组数据失效
     	wAddr = FLASH_FreeBufferAddr;//地址交换
...
void Flash_Write_Factory_KeySet(FLASH_KEY_TYPE ucType, WORD wIndex, WORD wBufSize, BYTE* buf, WORD wLen)
{
	...
	ucKeyIndex = Flash_GetKeyIndex(ucType, TRUE);//获取“空的”数据的状态位的数据
	
	if(ucType == FLASH_KEY_FACTORY)
	{
		wKEY_SIZE				= FACTORY_SIZE;//factory类型的大小
		wKEYSET_START			= FACTORY_START + (WORD)ucKeyIndex*FACTORY_SIZE;//存放的位置
		wKEYSET_BUF_START		= FACTORY_BUF_START;//freebuffer中存放数据的位置
		wKEYSET_STATUS_ADDR 	= FACTORY_STATUS_ADDR + ucKeyIndex;//存放数据状态位的位置
		wKEYSET_BUF_STATUS_ADDR = FACTORY_BUF_STATUS_ADDR;//freebuffer中存放数据状态位的位置
	}
#if 0    
	else if(ucType == FLASH_KEY_GAMMA)
	{
	...
	if( FlashReadByte( wKEYSET_STATUS_ADDR ) == FLASH_KEY_EMPTY || FlashReadByte( wKEYSET_STATUS_ADDR ) == FLASH_KEY_WRITE )
	{
		if( wIndex == 0x00 )
		{
			// FLASH_printData("  _FWFK_StatusAddr [%x] ",wKEYSET_STATUS_ADDR);
								//存放数据状态位的位置
			 FlashWriteByte(TRUE, wKEYSET_STATUS_ADDR, FLASH_KEY_WRITE);//修改buffer状态位
            g_wKEYSET_START         = wKEYSET_START;//存放的位置
		}
		Flash_WriteTbl( TRUE, wKEYSET_START + wIndex * wBufSize, buf, wLen );//将数据存进buffer
		if(( wIndex * wBufSize + wLen ) >= wKEY_SIZE ) // 检查是否写完数据
		{
			FlashWriteByte( TRUE, wKEYSET_STATUS_ADDR, FLASH_KEY_VALID );//给当前这组数据赋有效数据的标识
			if(ucKeyIndex>0)//如果不止一组数据把上一组数据状态位改为0x00，使它失效
				FlashWriteByte(TRUE, wKEYSET_STATUS_ADDR-1, FLASH_KEY_INVALID);//上组数据失效
		}
	}
	else
	{
		if( wIndex == 0x00 )
		{
			FlashSectorErase(TRUE, FLASH_FreeBufferAddr);//数据存满，擦除
			FlashWriteByte(TRUE, wKEYSET_BUF_STATUS_ADDR, FLASH_KEY_WRITE);//修改buffer状态位
            g_wKEYSET_START         = 0xF000;
		}
							//freebuffer中存放数据的位置
		Flash_WriteTbl( TRUE, wKEYSET_BUF_START + wIndex * wBufSize, buf, wLen );//将数据写进buffer
		if(( wIndex * wBufSize + wLen ) >= wKEY_SIZE ) // check write end//检测写进去的的数据大小和factory类型的数据大小是否一致
		{
								//freebuffer中存放数据状态位的位置
			FlashWriteByte( TRUE, wKEYSET_BUF_STATUS_ADDR, FLASH_KEY_VALID );//将buffer的数据的标志位改为0x22（有效）
			//FLASH_printData("  _FWFK_buffer write End [%x] ",ucType);
           	...
           	if( ucType >= FLASH_KEY_HDCP && ucType <= FLASH_KEY_DDCH )
			...
			#endif
				Flash_BackupFactorySetToBuffer(FBufType_FactorySetting);//将factory的数据搬到buffer，并交换地址
				...
				FlashSectorErase( TRUE, FLASH_FreeBufferAddr );    //charles test//擦除buffer
		}
...
/*
 *******************************************************************************
 * @Description ：从flash读取工厂设置的HDCP 、DDCA或者DDCD key
 * @Parameter1 ：ucType -> key类型
 * @Parameter2 ：bReadVaild -> 控制位，可以控制当数据无效时是否去读flash
 * @Parameter3 ：wIndex -> 偏移倍数
 * @Parameter4 ：wBufSize -> buf的大小
 * @Parameter5 ：*buf -> 存放数据的buf，指向数据所有设置
 * @Parameter6 ：wLen -> 要往buf写多长
 ********************************************************************************
 */
...
Bool Flash_Read_Factory_KeySet(FLASH_KEY_TYPE ucType, Bool bReadVaild, WORD wIndex, WORD wBufSize, BYTE* buf, WORD wLen)		//110311 Modify
{
	...
	if(bReadVaild)
		ucKeyIndex = Flash_GetKeyIndex(ucType, FALSE);//有效数据的状态位的位置
	else
		ucKeyIndex = Flash_GetKeyIndex(ucType, TRUE);

	if( ucType == FLASH_KEY_FACTORY )
	{
		wKEY_SIZE			= FACTORY_SIZE;//factory类型数据的大小
		wKEYSET_START		= FACTORY_START + (WORD)ucKeyIndex*FACTORY_SIZE;//存放的数据位置
		wKEYSET_STATUS_ADDR = FACTORY_STATUS_ADDR + ucKeyIndex;//存放数据状态位的位置
	}
#if 0    
	else if( ucType == FLASH_KEY_GAMMA )
	...
	else if( ucType == FLASH_KEY_DDCDP )
	{
		...
		return FALSE;
	//状态位是否为0xaa或0x22，是则将该状态位对应区域的数据读取出来
...
//////////////////////////////////////////////////////////////////////////////
//调Flash_Read_Factory_KeySet函数读数据，读到数据返回ture，没读到false；读到的数据放到pbuf，读ucLen个数据
...
//////////////////////////////////////////////////////////////////////////////
//查找Ident和CheckSum 位置为FLASH_EMPTYDATA (0xFF)的地址，得到一个没有被写过的地址Index,
//如果能找到这样一个Index,调用Flash_WriteTbl把MonitorSetting写入相应的地址，并把上一组保存在Flash中
//的Ident值写成FLASH_IDENTDATA_CLEAR (0x00)。
//注意事项：第一步如果不能找到这样一个Index,说明MonitorSetting区域已经写满了，不能再写入。
//调用FlashSectorErase 擦除FreeBuffer区，把MonitorSetting 一组值保存在FreeBuffer。(擦除FreeBuffer区这一步很重要)
//交换FreeBuffer和MonitorSetting区：把FreeBuffer区第一个Byte值写成FBufType_MonitorSetting (0x88)，把MonitorSettin区0x88写成0x00，
//把FreeBufferAddr赋值给MonitorSettingAddr，把原来的MonitorSettingAddr赋值给FreeBufferAddr，实现两个区块交换.
void Flash_WriteMonitorSet(BYTE* buf,BYTE len)
{
    ...
    if(i != IDENT_NOT_FOUND)// Found the empty index
    {
        ...
        if(i>0)
        {
            wAddr = MONITORSET_START + (i-1)*MONITORSET_SIZE;//并把上一组保存在Flash中的Ident值写成FLASH_IDENTDATA_CLEAR (0x00)。
            FlashWriteByte(TRUE, wAddr,FLASH_IDENTDATA_CLEAR);
        }
    }
    else
    {
		FlashSectorErase(TRUE, FLASH_FreeBufferAddr);//擦除FreeBuffer区
		Flash_WriteTbl(TRUE, MONITORSET_BUF_START, (BYTE*)buf, len);//把MonitorSetting 一组值保存在FreeBuffer
		FLASH_printData("_FWMS_1 wAddr %x",MONITORSET_BUF_START);
		FLASH_printData("FLASH_FreeBufferAddr11 wAddr %x",FLASH_FreeBufferAddr);
		FlashWriteByte(TRUE, FLASH_FreeBufferAddr,FBufType_MonitorSetting);//把FreeBuffer区第一个Byte值写成FBufType_MonitorSetting (0x88)
...
void Flash_WriteModeSet(BYTE index,BYTE *buf)
{
	...
	for(i=0;i < MAX_MODESET_NUM; i++)//从第0组开始查找整个ModeSetting区的各组Mode数据
	{	//读取各组有效的Mode数据,查找是否已有保存此Mode
		if(FlashReadByte(MODESET_START+i*MODESET_SIZE)==FLASH_IDENTDATA && FlashReadByte(MODESET_START+i*MODESET_SIZE+MODESET_SIZE-1)==FLASH_CHECKSUM)
		{
			if(FlashReadByte(MODESET_START+i*MODESET_SIZE+1)==index)//若存在此Mode
			{
				FlashWriteByte(TRUE, MODESET_START+i*MODESET_SIZE, 0x00); // Clear Identdata.//把已存在的相同Mode的Ident写成0x00
    ...
    {	
        if(Flash_CheckModeFull())//检查Mode有效数据是否存满
            Flash_ClearUserModeSpace();//若存满，则释放UserMode空间(将UserMode Ident写成0x00)

        FlashSectorErase(TRUE, FLASH_FreeBufferAddr);//擦除FreeBuffer空间
		for(i=0;i < MAX_MODESET_NUM; i++)//从第0组开始查找整个ModeSetting区的各组Mode数据
		{	
		//依次把Ident为0x55和CheckSum为0xAA的有效数据保存到FreeBuffer空间
			if(FlashReadByte(MODESET_START+i*MODESET_SIZE)==FLASH_IDENTDATA  && FlashReadByte(MODESET_START+i*MODESET_SIZE+MODESET_SIZE-1)==FLASH_CHECKSUM)
			{
        ...
        if(j < (MAX_MODESET_NUM-1))// 将要保存的mode数据存在FreeBuffer区的有效mode数据后
        {
        ...
        //	Exchange Buffer
		FlashWriteByte(TRUE, FLASH_FreeBufferAddr,FBufType_TimingMode);//保存完后，把FreeBuffer空间设置成ModeSetting空间,交换地址
...
Bool Flash_ReadModeSet(BYTE index,BYTE *buf)
{
	...
	Flash_ReadTbl(MODESET_START+i*MODESET_SIZE, buf, MODESET_SIZE);//将对应的mode读取出来
    return TRUE;
}
