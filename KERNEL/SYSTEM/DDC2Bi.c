...
Bool JRYColorToolControl (void)
{
	...
	else if((DDCBuffer[3] == 0x39) && (DDCBuffer[4] == 0x31)) //3DDC����
	{
		BYTE  j, i = 0;
		j = DDCBuffer[5];
	...
	else if((DDCBuffer[3] == 0x39) && (DDCBuffer[4] == 0x32)) //3ddC У��
	{
		BYTE  checksun = 0;
		WORD  i;
	...
	else if((DDCBuffer[3] == 0x39) && (DDCBuffer[4] == 0x33)) //3ddc����
	{
		BYTE  checksun;
		BYTE  EDID_TYPE = 0;
		#if USEFLASH
		...
		#endif
		checksun = DDC1[0x7F]; //2  ֻ��У��128 �ֽ�
	...
	else if((DDCBuffer[3] == 0x39) && (DDCBuffer[4] == 0x30)) //DDC����
	{
	...
	else if((DDCBuffer[3] == 0x39) && (DDCBuffer[4] == 0x31)) //DDC����
	{
		BYTE  j, i = 0;
		j = DDCBuffer[5];
		if (edid_pointer < edid_cont)
	...
	else if((DDCBuffer[3] == 0x39) && (DDCBuffer[4] == 0x32)) //2    ddC У��
	{
		BYTE  checksun = 0;
		WORD  i;
		if ((edid_cont == 128) || (edid_cont == 256))
	...
	else if((DDCBuffer[3] == 0x39) && (DDCBuffer[4] == 0x33)) //ddc����
	{
		BYTE  checksun;
		BYTE  EDID_TYPE = 0;
		#if USEFLASH
...
#message "please check , dont  have this function"
	...
	else if((DDCBuffer[3] == 0x39) && (DDCBuffer[4] == 0x35)) //HDCP  ����
	{
		BYTE  j, i = 0;
		j = DDCBuffer[5];
		if(DDCCI_pointer < DDCCI_cont)
		{
			...
			if(DDCCI_pointer <= (DDCCI_cont - 1)) //2<=  ��֤�������һ�η���У����
			{
				DDCBuffer[0] = 0x84;
				DDCBuffer[1] = 0x00;
				DDCBuffer[2] = 0x4F;
	...
	else if((DDCBuffer[3] == 0x39) && (DDCBuffer[4] == 0x36)) //HDCP У��
	{
		WORD  wCount ;
		BYTE   i = 0;
		//�ڴ˴��Ա�������ݽ��ж�ȡ�����У������ǰ���յ�У��������OK,����NG.
		checksun_verify = 0;
		for(i = 0; i < 20; i++)
			DDC1[i] = 0;
		#if USEFLASH
...
//*******************************************************************************************
	...
	if (CommandCode == GETVCPFeature || CommandCode == GETPagedVCP)
	{
			...
			// 2-20
		{
			...
			// Jeff add it in 2004?~7��13��?U�� 04:46:32
		{
			ValueH = 0xFF;
			ValueL = 0x16;
			RetValueH = 0;
