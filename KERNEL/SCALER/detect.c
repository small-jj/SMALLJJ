...
void mStar_MonitorInputTiming(void)
{
    ...
    // detect whether input signal is changing             (3)
    if (!InputTimingChangeFlag)//有/无信号(1)InputTimingChangeFlag为1，不进入,跳else(2)InputTimingChangeFlag为0，进入
    {							
        if (SyncLossState())//	(2)SyncLoss没有改变，条件成立进入
        
        {	 //FreeRunModeFlag代表着屏幕没信号，给屏幕的一种状态，防止看出屏幕花
        
            if (!FreeRunModeFlag)//（2）FreeRunModeFlag为1，判断条件为0，不成立
            {
                Set_InputTimingChangeFlag();
            }
            else if (mStar_SyncLossStateDetect())//（2）无信号进入，有信号出来，返回true，条件成立
            // detect mode while no Sync/suspend/standby/disconnect
            {
                DETECT_printMsg("no sync -> have sync");
                Set_InputTimingChangeFlag();					//（2）状态发生改变，也就是信号发生改变，调出if
            }
    #if Dual
        ...
        // valid timing
        {
            ...
            // check if input timing is changing检测信号是否发生改变
            if (mStar_ValidTimingDetect())//当信号变化时这里和中断可知道 //(3)返回false
            {
                SrcFlags |= SyncLoss;
                Set_InputTimingChangeFlag();
...
#endif
#endif
//msWriteByteMask( REG_1F15,0,BIT5);	//1 �� ��DP FREESYNC ״̬����, �� ����֤,��ʹ��
            }
        }
    }
    //=============================================================================
    //=============================================================================
    if (InputTimingChangeFlag)//信号不改变为0，信号状态发生改变这里一定为1（2）进来													//my
...
#endif	 //状态发生改变进入   
        mStar_PrepareForTimingChange();													//（1,2）进入
    }
    else																				//(1,2)不走														
    ...
    {														//(3)进来
        ...
        if (!InputTimingStableFlag)						//62次后一直为1
        {
	//130625 nick
    SpeedupOrExtendSwitchPort();
    		...
    		//有信号 ValidTimingStableCount，vga第12次时进入，DVI第22次进入，HDMI第62次进入，DP第62次进入
    		//无信号 SyncLossStableCount,VGA第21次是进入->HDMI，DVI 91  ; HDMI 151->VGA ; DP 231
            ...
            {
                Set_InputTimingStableFlag(); // set flag to search for input mode									//my
                Set_DoModeSettingFlag();																			//my
    ...
    printData("SrcInputType:%d", SrcInputType);
                ...
                if(SyncLossState() && (UserPrefInputSelectType == INPUT_PRIORITY_AUTO) && 				//my
                    (++SwitchPortCntr <= Input_Nums)//vga+hdmi = 2					//条件成立，切换端口
                    ) // 100125 coding
                {
                    Clr_InputTimingStableFlag(); // set flag to search for input mode
                    Clr_DoModeSettingFlag();
                    SrcInputType = (SrcInputType + 1) % Input_Nums;//HDMI 1  VGA 0 HDMI下，（1+1）% 2 =0，则HDMI切到VGA
                    ...
                    //====Test 20130312 ====//
                    #endif
                    mStar_SetupInputPort();//切换端口										端口切换后进入while(1)
...

{
	...
	printData( " TMDS_DVI Color:%x", msReadByte( REG_0580 ));
        ...
        if (!ReadyForChangeFlag)
        {
            ...
            //    CheckADC();
            if (!mStar_FindMode())																				//my  find mode
            // search mode index
            {
        ...
        //supported mode
        {
            if (!ReadyForChangeFlag)
            {
                ReadModeSetting(); // restore user's settings from NVRAM for each mode							//my read mode
		...
		#endif
            if (mStar_SetupMode() == FALSE)																		//my setup mode
            // setup scaler
            {
            	DETECT_printMsg("Setup Mode False!");
                Set_InputTimingChangeFlag(); // setup mode failed
                mStar_SetupFreeRunMode(); // setup freerun mode//FreeRunModeFlag=1
...
void mStar_PrepareForTimingChange(void)
{
  ...
  if(CURRENT_INPUT_IS_DVI())
    ...
    Clr_ForcePowerSavingFlag();//(1)ForcePowerSavingFlag = 0强制省电保存标志							//my（1）
    PowerDownCounter = 0;
    InputColorFormat = INPUTCOLOR_RGB;
#if DisplayPowerOnLogo	
    if (!DisplayLogoFlag)
#endif		
    {
        if (PowerSavingFlag)//判断是否开机状态	1为关机												//my
        {
            Power_PowerOnSystem();
            mStar_SetupFreeRunMode();//(2)SetupFreeRunModeflag = 1
         }
        else//------------------------------------------------------------------------	//(1，2)来这
        {
            //if (PanelOnFlag)
    ...
    if (InputTimingStableFlag)		//（1,2）为·0					//63次条件成立							//my
    {	//重置图形端口，重新计数输入 hfreq 和 vtotal
        SC0_SCALER_RESET(GPR_B);//msWriteByte(SC0_F1, GPR_B); 
        ForceDelay1ms(3);
	...
	//复位输入时序稳定，等待时序稳定
	Clr_InputTimingStableFlag(); // （1,2）InputTimingStableFlag =0    									    my
	InputTimingStableCounter = 0;			//(1,2)InputTimingStableCounter = 0																			//my
    SrcFlags &= ~(bUnsupportMode | bUserMode | bNativeMode);//（1,2）SrcFlags 高三位清零                                          
...
Bool mStar_SyncLossStateDetect(void)
{
    ...
    else														//（2）bit2 bit 3被清掉 SrcFlags非syncloss状态，返回true
    {
    ...
    fStatus = mStar_GetInputStatus();
    if( fStatus & SyncLoss )        // no sync//有信号不走这里
    {
    ...
    {	//mStar_SyncLossStateDetect中可得到SrcFlags，两者判断不等说明信号不稳定或切换分辨率   
        if( SyncPolarity( SrcFlags ) != SyncPolarity( fStatus ) )    //将这次的极性和上一次作对比        // Sync polarity changed
        {
            SrcFlags = ( SrcFlags & 0xF0 ) | fStatus; // clear others flags, except sync informat (polarity & w/o)
...
BYTE mStar_GetInputStatus(void)
{
    ...
    {	//读Hperoid（水平周期频率）																					my
       ...
       if( inputValue == 0x1FFF || inputValue < 10 )//判断信号是否值是否等于0x1fff或小于10，是信号无效
       {
           NOSYNC_printMsg(" H peroid is Out of range!");
           fStatus |= bHSyncLoss; //对应bit2
       }
    }
#if ENABLE_FREESYNC
    if(!IS_HDMI_FREESYNC())
    ...
    if( inputValue == SC_MASK_V || inputValue < 200 ) // 130530 JUN Modify//判断信号是否值是否等于0xfff或小于200，是信号无效
    {
        fStatus |= bVSyncLoss;//对应bit3
    }
    }
//	DETECT_printData(" V total:  %d",inputValue);
    ...
    fStatus |= ( status & 0x03 ); // Get input timing polarity//保存bit0 bit1及极性                              my
#if ENABLE_FREESYNC
        if(IS_DP_FREESYNC())
            {
            if(!(status&CSP_B))
...
#endif
	//CURRENT_INPUT_IS_TMDS判断什么信号											(2，3)有信号，走if
    if(CURRENT_INPUT_IS_TMDS())//( SrcInputType == Input_Digital || SrcInputType == Input_Digital2 ) 
    {
        if(DVI_DE_STABLE() == FALSE)//数字使能信号，没信号为false
        {
    ...
    else if( SyncLossState() && !( fStatus & SyncLoss ) )        // 当当前状态为无同步时，输入时序有效
    {
        Delay1ms( 20 );
        if( InputTimingChangeFlag )
        {
