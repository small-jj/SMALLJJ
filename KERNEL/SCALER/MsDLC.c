...
#define DefAveLumi        BRIGHTNESS_PWM_DEF_VALUE//55
#define MaxAVeLumi       BRIGHTNESS_PWM_MAX_VALUE //7a
#define MinAveLumi        BRIGHTNESS_PWM_MIN_VALUE//00
...
void msDCRHandler(void)
{
    ...
    WORD xdata AveLumi , AveLumi2;
#endif
	//当开启动态对比度UserPrefDcrMode = 1时，不执行
    if ( !UserPrefDcrMode )
    {
        return;
    }
	...
	//读取画面中黑点所占的DATA数和亮点所占的DATA数
	BlackDataTemp = Y_CHANNEL_MAX_PIXEL();
    WhiteDataTemp = Y_CHANNEL_MIN_PIXEL();
    //比较当前与上一次的平均灰度是否相差大于10，大于则说明
    //画面不稳定，则更新数据(现在的赋给以前的)结束函数，进
    //入下次循环，小于10返回不作操作
    //
	...
	//如果画面中最亮像素点所在的灰阶不等于255
    if ( ( tempVal = Y_CHANNEL_MAX_PIXEL() ) != BrightData ) //(( tempVal = msReadByte( SC7_3A ) ) > BrightData ) // get max bright data
    {
        BrightData = tempVal;//更新当前最亮点的值
    }
	//如果画面中最暗素点所在的灰阶不等于0
    if ( ( tempVal = Y_CHANNEL_MIN_PIXEL() ) !=DarkData ) //(( tempVal = msReadByte( SC7_39 ) ) < DarkData ) // get min dark data
    {
        DarkData = tempVal;//更新当前最暗点的值
    }
	/*
		  判断当前画面的最大亮度和最小亮度的差值是否小于设定的黑白差距值
		  如果小于，进一步判断当前画面最小亮度和定义的最小亮度值的差值是否小于设定的最黑的范围
		  成立则把SetDarkestFlag标志位置1
	  */
	
    if ( abs ( BrightData - DarkData ) <= BlackWhiteDifference )
    {
        if ( abs ( DarkData - DarkestValue ) <= DarkestRange ) // all display data are dark 20/30
        {
            SetDarkestFlag = 1;
        }
    }
    ...
    /*
	#define ClearBalclightStartBit							BIT4//0x10
    	SystemFlags初始值0
	...
	#define Clr_EanbleClearBalclightStartFlag() 			(SystemFlags&=~ClearBalclightStartBit)
    	判断当前画面的最大亮度是否小于设定的最黑范围，同时平均灰度值小于16，同时当前画面最小亮度小于16
    	如果成立，进一步判断EanbleClearBalclightStartFlag，成立，5个计数单位后关闭背光，同时将SystemFlags的BIT4置1
    	如果不成立，不执行倒计时开背光，把SystemFlags的BIT4置0，SystemFlags的BIT5清0
	*/

        if(BrightData<=DarkestRange&&AveLumi<=16&&DarkData<=16)
            {

              if(!EanbleClearBalclightStartFlag)//0为没有置起标志位
                {
		    	DCR_Counter=5;						
		    	Set_EanbleClearBalclightStartFlag();//置起标志位,关闭背光
                }
            ...
            {
                  DCR_Counter=0;						
	           Clr_EanbleClearBalclightStartFlag(); //qing 0
                  Clr_DoClearBalclightFlag(); //qing 0
            } 
	//当前画面的平均灰度是否大于255，是就等于255，否就等于当前画面平均灰度
...
printData("AveLumi             %d", AveLumi);
    ...
    AveLumi =(AveLumi*100/0xFF);			//for HF Panel  һ������240ma��������OxFF
    BackLightTargetValue=AveLumi;
#else
	//判断当前画面灰度与默认灰度的大小，并给定目标值

	//计算平均灰度，让平均灰度值与亮度值挂钩
    AveLumi =AveLumi*(MaxAVeLumi-MinAveLumi) /0xFF;			//for HF Panel  һ������240ma��������OxFF
...
#endif
//PWM调幅的最大值为0x00到0xFF，调节的是电压值，当亮度为100的时候，按照等比例转换成十六进制，PWM的值未必是0xFF，
//可能在该屏中最大值为0x7A，如果再加大可能会导致通过电压过大屏幕烧坏。
//如果想要在亮度为100时更亮，可将获取平均灰度值的比例增大，在限流范围内，否则会导致屏幕烧毁
...
/*
	...
	判断周期计数变量，成立->继续判断背光目标值减去上次背光的值是否大于DCR步长
	是->则把DCR调整背光标志位bDCRSmoothMotionFlag置1，步长调节的值为DCRStep / 2，即为1
	否->则把DCR调整背光标志位bDCRSmoothMotionFlag清0。
*/
//调整亮度值
 if ( !DCRStepTransCnt )
{
    if ( abs ( BackLightTargetValue - BacklightPrev ) >= DCRStep )//大于定义的值（周期计数变量），则背光改变，置起标志位
    {
        bDCRSmoothMotionFlag = 1;
		//DCR转换值
	...
	/*判断bDCRSmoothMotionFlag，1则说明当前背光亮度不等于背光目标值，需要更新当前背光值BacklightNow，0则说明当前背光亮度不需要调节。
    	1：如果背光目标值大于等于上次背光值，当前背光亮度加上步长值如果小于背光目标值，当前背光亮度等于自生加步长值
           如果背光目标值小于等于上次背光值，当前背光亮度减去步长值如果大于背光目标值，当前背光亮度等于自生减步长值
    	0：当前背光亮度等于目标背光值

    	DCRStepTransCnt = DCRStepTransPeriod;
    	重新赋值周期计数变量DCRStepTransCnt，DCRStepTransPeriod为1，即10ms调节一次背光
	#if  EANBLE_NEW_DCR_Functing
        if(( ms_Counter % 100 ) == 0 )
        {
            if ( DCRStepTransCnt )
	...
	#endif
	*/
	//如果背光目标值大于之前的背光值，那么它就需要逐渐增加1，反之逐渐减1，所做的功能就是将当前的背光值等于背光目标值
    if ( bDCRSmoothMotionFlag ) // 背光改变 DCR operating range is over DCRStep
    {
        if ( BackLightTargetValue >= BacklightPrev ) // increasement
        {
            BacklightNow = ( BacklightNow + DCRStepTransValue >= BackLightTargetValue ) ? ( BackLightTargetValue ) : ( BacklightNow + DCRStepTransValue );
...
#if DCRMotionTest
            DCRStepTransCnt = DCRStepTransPeriod;//DCRStepTransPeriod = 1
#endif
...
#if DCRMotionTest
   /*
	   如果当前的背光值不等于上一次的背光值或者最大亮度的值不等于上一次最大亮度的值，
	   让上一次背光值等于当前的背光值、上一次最大亮度值等于当前最大亮度值
   */
   ...
   // if ( !DCRStepTransCnt )
    ...
    HistogramPrev = HistogramNow;//更新直方图
    
#if defined(DCRClearBacklight)
		//如果关背光的标志被置起来，且当前画面是RootMenu，则关背光
		...
		//判断背光工作标志
			if(BackLightActiveFlag)
				hw_ClrBlacklit();//清除背光工作标志，关背光
		}
		if((!DoClearBalclightFlag)||MenuPageIndex>RootMenu)//如果清除背光标志为假，或者菜单大于根菜单时
		{
			if(!BackLightActiveFlag)
				hw_SetBlacklit();//置起背光工作标志，开背光
		}	
		/*
	#define hw_SetBlacklit()	(MEM_MSWRITE_BIT(_REG_GPIO0_OEZ, 0, BIT3),MEM_MSWRITE_BIT(_REG_GPIO0_OUT, 0, BIT3), BackLightActiveFlag=1)	  
	#define hw_ClrBlacklit()	(MEM_MSWRITE_BIT(_REG_GPIO0_OEZ, 0, BIT3),MEM_MSWRITE_BIT(_REG_GPIO0_OUT, 1, BIT3), BackLightActiveFlag=0)
			假如当前为关闭背光的状态，必定走的是hw_ClrBlacklit，背光动作BackLightActiveFlag必然为0，if(BackLightActiveFlag)条件必然不成立，
			如果要打开背光，清理背光的标志位为0，if(!BackLightActiveFlag)条件必然成立，就可以hw_SetBlacklit()打开背光。
		
			假如当前为打开背光的状态，必定走的是hw_SetBlacklit，背光动作BackLightActiveFlag必然为1，if(!BackLightActiveFlag)条件必然不成立，
			如果要关闭背光，清理背光的标志位为1，if(BackLightActiveFlag)条件必然成立，就可以hw_ClrBlacklit()关闭背光。
		*/
