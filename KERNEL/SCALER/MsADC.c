...
static BYTE _msADC_CheckPGAGain(BYTE vsyncTime);    //110401 Modify
...
//
//**************************************************************************
void msADC_SetPhaseCode(BYTE phasecode)
{
...
//
//**************************************************************************
void msADC_SetRGBGainCode(WORD rcode, WORD gcode, WORD bcode)
{
...
//                  
//**************************************************************************
void msADC_SetRGBOffsetCode(WORD rcode, WORD gcode, WORD bcode)    //110401 Modify
{
...
///////////////////////////////////////////////////////////////////////////////
/// @details    Set Adc Pga gain
/// @param[in]  ucAdcPgaGain is pga gain to set, pgagain value 0->0x15->0x2A
/// @pre        drvAdcInit() and drvAdcAutoColor() has been invoked.
///////////////////////////////////////////////////////////////////////////////
void msADC_SetPgaGain   ( BYTE ucAdcPgaGain )    //110401 Modify
{
    _msADC_Write2BytesMask( REG_ADC_DTOP_55_L, ucAdcPgaGain, 0x03F );
}
...
void msADC_ADCOffsetGainMismatchCal(void)
{
    ...
    _msADC_Write2BytesMask(REG_ADC_ATOP_5B_L, wGch1diff, 0x07FF); // Set ADC channel 1 offset compensated code	//110608 Modify
    ...
    _msADC_Write2BytesMask(REG_ADC_ATOP_5F_L, wBch1diff, 0x07FF); // Set ADC channel 1 offset compensated code	//110608 Modify
...
#else
{
    ...
    {
        msADC_SetPhaseCode(adjPhase);
        Delay1ms(vsyncTime);	//110316 Modify
        _msADC_WaitAutoStatusReady(ATPCTRL, ATPR_B);
        cksum=mStar_ReadWord(ATPV4);
        cksum=(cksum<<16)|mStar_ReadWord(ATPV2);
        if (cksum>maxcksum)
...
{
    BYTE result, ucBank;
    BYTE xdata ucAdcPgaGain;    //110401 Modify 

    if (SrcInputType==Input_Digital)
        return FALSE;

    ADC_DPUTSTR("msADC_AutoAdcColor\n");
    ucBank = mStar_ReadByte(REGBK); // backup bank
    mStar_WriteByte(REGBK, REGBANKSCALER);
    //110401 Modify
    mStar_WriteByteMask(BK0_0F, 0x00, BIT7);    // Enable Auto Rose Func 

    // Set ADC offset and gain to default value
    msADC_SetRGBGainCode(0x400, 0x400, 0x400);
    //if((bInputIsSOGFlag && bInputSOGisYUV) ||bInputVGAisYUV) //YPbPr or YUV_H+V
        //drvAdcSetOffsets(0x200, 0x40, 0x200);
    //else
    msADC_SetRGBOffsetCode(0x000, 0x000, 0x000);

    mStar_WriteByte(ATGCTRL, 0x01); // enable auto Gain
    ucAdcPgaGain = _msADC_CheckPGAGain(vsyncTime);    //110401 Modify
    result = _msADC_AutoAdjustAdcGain(vsyncTime);
    if (result)    //110401 Modify 
    {
        UserPrefAdcPgaGain = ucAdcPgaGain;
    ...
    #if 1
    ADC_DPRINTF("adc gain red %d", UserPrefAdcRedGain);
    ADC_DPRINTF("adc gain green %d", UserPrefAdcGreenGain);
    ADC_DPRINTF("adc gain blue %d", UserPrefAdcBlueGain);
    #endif
...
static void _msADC_BackupADCSetting(void)
{
    ...
    gADCBackupSetting.Blacklevel_Rch = (_msADC_ReadByte(REG_ADC_DTOP_44_H) << 8 | _msADC_ReadByte(REG_ADC_DTOP_44_L)) & 0x03FF;	//110608 Modify
    gADCBackupSetting.Blacklevel_Gch = (_msADC_ReadByte(REG_ADC_DTOP_47_H) << 8 | _msADC_ReadByte(REG_ADC_DTOP_47_L)) & 0x03FF;	//110608 Modify
    gADCBackupSetting.Blacklevel_Bch = (_msADC_ReadByte(REG_ADC_DTOP_50_H) << 8 | _msADC_ReadByte(REG_ADC_DTOP_50_L)) & 0x03FF;	//110608 Modify
    gADCBackupSetting.Offset_Rch = (_msADC_ReadByte(REG_ADC_DTOP_46_H) << 8 | _msADC_ReadByte(REG_ADC_DTOP_46_L)) & 0x07FF;		//110608 Modify
    gADCBackupSetting.Offset_Gch = (_msADC_ReadByte(REG_ADC_DTOP_49_H) << 8 | _msADC_ReadByte(REG_ADC_DTOP_49_L)) & 0x07FF;		//110608 Modify
    gADCBackupSetting.Offset_Bch = (_msADC_ReadByte(REG_ADC_DTOP_52_H) << 8 | _msADC_ReadByte(REG_ADC_DTOP_52_L)) & 0x07FF;		//110608 Modify
}
...
static void _msADC_SetFreeRunClock(void)
{
    ...
    _msADC_Write2BytesMask( REG_ADC_ATOP_0A_L, 0x0010, 0x003F ); // [5]:DIV2_EN = 1'b0, [4:3]:APLL_MOD=2'b10, [2:0]: APLL_MULT = 0
    _msADC_Write2BytesMask( REG_ADC_ATOP_09_L, 0x0200, 0x0300 ); // ADCPLL_ICTRL = 2'b10
    _msADC_Write2BytesMask( REG_ADC_DTOP_02_L, 0x6656, 0xFFFF );
    _msADC_Write2BytesMask( REG_ADC_DTOP_03_L, 0x0066, 0xFFFF );
    _msADC_Write2BytesMask( REG_ADC_DTOP_06_L, 0x0080, 0x0080 ); // PLL Frequency Force Enable
#endif
}
...
//                  TRUE: done, FALSE: fail
//**************************************************************************
static Bool _msADC_AutoAdjustAdcGain(BYTE vsyncTime)	//110527 Modify
{
    ...
    while (retry--)
    {
        ...
        msADC_SetRGBGainCode(rGain, gGain, bGain);
        Delay1ms(vsyncTime);
        // Wait scaler auto gain is done, BK0_78[1] = 1
        _msADC_WaitAutoStatusReady(ATGCTRL, ATGR_B);
...
static BYTE _msADC_CheckPGAGain(BYTE vsyncTime)    //110401 Modify
{
    BYTE xdata ucBank, u8CurPGA, adcStatus;

    ucBank = mStar_ReadByte(REGBK); // backup bank
    u8CurPGA = 0x00;//u8CurPGA = _msADC_ReadByte(REG_ADC_DTOP_55_L) & 0x3F;     //110401 Modify 
    _msADC_Write2BytesMask(REG_ADC_DTOP_55_L, u8CurPGA, 0x003F);       //110401 Modify 

    mStar_WriteByte(REGBK, REGBANKSCALER);
    mStar_WriteByteMask(ATGCTRL, BIT3, BIT3); // outpu is overflow or underflow result
    Delay1ms(vsyncTime);    //110401 Modify
    // Wait scaler auto gain is done, BK0_78[1] = 1
    _msADC_WaitAutoStatusReady(ATGCTRL, ATGR_B);
    adcStatus=mStar_ReadByte(ATGCTRL);
    ...
    mStar_WriteByteMask(ATGCTRL, 0, BIT3);
    mStar_WriteByte(REGBK, ucBank);        // Switch to Original bank

    return u8CurPGA;    //110401 Modify
}
...
static void _msADC_EnableScalerPG(msADCScalerPG_HtotalSel ht_sel)
{
    ...
    //mStar_WriteByteMask(ISELECT, 0x00, 0x80); // disable free-run
    mStar_WriteByteMask(DBFC, 0, 0x01); // disable double buffer
    mStar_WriteByteMask(ISELECT, SCALER_ISEL_ANALOG1|BIT2, 0x07); // [2]:use Hsync only, [1:0]:ISEL = analog1
    mStar_WriteWord(SPRHST_H, 0x0010); // SPRHST = 0x10
    whact = 256*(ht_sel+1) - 16;
    mStar_WriteWord(SPRHDC_H, whact); // SPRHDC = htsel - 0x10
    // Vtotal is 64 lines, Vstart = 0x10, Vactive = 0x30
    mStar_WriteWord(SPRVST_H, 0x0010); // SPRVST = 0x10
    mStar_WriteWord(SPRVDC_H, 0x0030); // SPRVDC = 0x30
    mStar_WriteByte(BK0_00, REGBANK1);
...
static void _msADC_BackupScalerSetting(void)
{
    ...
    mStar_WriteByte(REGBK, REG_BANK_SCALER);        // Switch to Original bank
    gScalerBackupSetting.DBuffer = mStar_ReadByte(DBFC);
    gScalerBackupSetting.ISEL = mStar_ReadByte(ISELECT);
    gScalerBackupSetting.SPRHST = mStar_ReadWord(SPRHST_H) & 0x07FF;
    gScalerBackupSetting.SPRHDC = mStar_ReadWord(SPRHDC_H) & 0x0FFF;
    gScalerBackupSetting.SPRVST = mStar_ReadWord(SPRVST_H) & 0x07FF;
    gScalerBackupSetting.SPRVDC = mStar_ReadWord(SPRVDC_H) & 0x07FF;
...
static void _msADC_RecoverScalerSetting(void)
{
    ...
    mStar_WriteByte(REGBK, REG_BANK_SCALER);        // Switch to Original bank
    mStar_WriteByteMask(DBFC, gScalerBackupSetting.DBuffer, 0xFF);
    mStar_WriteByteMask(ISELECT, gScalerBackupSetting.ISEL, 0xFF);
    mStar_WriteWord(SPRHST_H, (mStar_ReadWord(SPRHST_H) & 0xF800) | gScalerBackupSetting.SPRHST);
    mStar_WriteWord(SPRHDC_H, (mStar_ReadWord(SPRHDC_H) & 0xF000) | gScalerBackupSetting.SPRHDC);
    mStar_WriteWord(SPRVST_H, (mStar_ReadWord(SPRVST_H) & 0xF800) | gScalerBackupSetting.SPRVST);
    mStar_WriteWord(SPRVDC_H, (mStar_ReadWord(SPRVDC_H) & 0xF800) | gScalerBackupSetting.SPRVDC);
...
static Bool _msADC_IsReadLineBuffer(Bool bflag)
{
    ...
    if(bflag)
    {
        ...
        {
            ForceDelay1ms(10);
            if(mStar_ReadByte(BK5_32) & BIT7) // Check ready bit for reading line buffer
            {
...
static msADCAvgVal _msADC_GetRGBLineBufAverageData(WORD wLineBufAddress, BYTE LineBuf, BYTE AvgCnt)
{
    ...
    for(i=0; i<AvgCnt; i++)
    {
        if(i%2 == 0)
        {
            ...
            wAvgBdata += mStar_ReadByte(BK5_35); //read line buffer data
            // G
            mStar_WriteByteMask(BK5_33, 1, 0x07); // select G0 to read
            wAvgGdata += mStar_ReadByte(BK5_35); //read line buffer data
            // R
            mStar_WriteByteMask(BK5_33, 2, 0x07); // select R0 to read
            wAvgRdata += mStar_ReadByte(BK5_35); //read line buffer data
        }
        else
        {
            // B
