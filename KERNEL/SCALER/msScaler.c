...
//setup wImgSizeOutH & wImgSizeOutV ---begin
    ...
    if( (UserprefExpansionMode!=Expansion_Full) && (g_SetupPathInfo.ucSCFmtIn == SC_FMT_IN_NORMAL) )
    {
    ...
    switch(g_SetupPathInfo.ucSCFmtIn)
    {
        case SC_FMT_IN_NORMAL:
           ...
           // PRINT_DATA("\r\n g_SetupPathInfo.wImgSizeInH 1=%x", g_SetupPathInfo.wImgSizeInH);
            ...
            if(SrcFlags & bInterlaceMode)//(SRC_FLAGS & INTERLACE_MODE_BIT)
...
#if Enable_Expansion && (FRAME_BFF_SEL==FRAME_BUFFER && !ASPECT_RATIO_CASE34)//JisonWideMode
    if( (UserprefExpansionMode!=Expansion_Full) && (g_SetupPathInfo.ucSCFmtIn==SC_FMT_IN_NORMAL))
    {
