...
// 111004 coding check with Shadow
    ...
    if(bEnable)
    {
        ...
        msWriteByteMask(REG_PM_B2, 0x00, BIT2);//Not power down CLKIN(A/B)[2]
        msWriteByteMask(REG_PM_8F, 0x00, 0x0F);// DVI RAW clock (A/B) pass
        msWriteByteMask(REG_PM_8C, 0x00, 0x0F);// DVI RAW clock (C)pass  ѡ��C prot clock  20180822 ��Ң
  #else
        msWriteByteMask(REG_PM_B3, 0x00, BIT0);//Not power down RCK(B)[1]
        msWriteByteMask(REG_PM_B3, 0x00, BIT2);//Not power down CLKIN(C)[2]
        msWriteByteMask(REG_PM_8C, 0x00, 0x0F);// DVI RAW clock (C)pass
...
{   
    ...
    msWriteByteMask( REG_1207, BIT4, BIT4 ); // �����ʺA�ٹq�Ҧ�
...
*/		//<<<DVI---------------------------------------------------------------------------------
    ...
    #if ((FRAME_BFF_SEL == FRAME_BUFFER) || (ENABLE_RTE))&&(CHIP_ID != CHIP_TSUMF)
    {
        ...
        msWriteByteMask( REG_1207, 0, BIT4 );   // ���}�ʺA�ٹq�Ҧ�
#endif
        msInitMemory();
    }
#endif
...
void
msPM_WaitingEvent(void)//在没有按键事件触发，一直在这里运行while（1），该函数不检测信号，只检测按键
{
    BYTE GetKeyTemp = 0;
   ...
   // msPM_InterruptEnable(FALSE);
	...
	if( !TPDebunceCounter )
	{
            ...
            if(PM_CheckKeypadStatus(GetKeyTemp))											//检测到按键跳出
                break;
	}
...
void  msPM_Handler(void)
{
    switch(sPMInfo.ePMState)
    {
        ...
        case ePM_WAIT_EVENT://关机状态
            
            msPM_WaitingEvent();//等待按键触发事件
            if(msPM_Checkagain())//防误触
                sPMInfo.ePMState = ePM_EXIT_PM;//按键开机，按键有效，退出PM
            else
                sPMInfo.ePMState = ePM_ENTER_PM;			
        break;

        case ePM_EXIT_PM://按键开机
   ...
   printMsg("ePM_EXIT_PM   Enter");
	...
	//再次检查，移至此处进行断电设置 
            ...
            if(PowerOnFlag) // wakeup form power saving	//只是按键按下跳出while，刚开始还没开机，不成立走else								//my
            {
